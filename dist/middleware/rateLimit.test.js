...
const mocks = vi.hoisted(() => {
    ...
    return {
        cache,
        incrementCache: vi.fn(async (key) => {
        ...
        setCacheExpire: vi.fn(async (key, seconds) => {
            const entry = cache.get(key);
            if (entry)
                entry.expireAt = Date.now() + seconds * 1000;
        }),
        getCacheTTL: vi.fn(async (key) => {
            const entry = cache.get(key);
...
function makeInteraction(overrides) {
    ...
    return {
        ...
        user: { id: overrides.isOwner ? "u_owner" : "u1", tag: "user#0001" }, member: overrides.isAdmin
            ? { permissions: { has: (f) => f === PermissionsBitField.Flags.Administrator } }
