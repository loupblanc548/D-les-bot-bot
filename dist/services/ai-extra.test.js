...
import { translateText, summarizeMessages, getSupportedLanguages, getLanguageName, } from "./ai-extra.js";
describe("ai-extra", () => {
    beforeEach(() => {
        vi.clearAllMocks();
    });
    ...
    describe("translateText", () => {
        ...
        it("should use configured model", async () => {
            ...
            expect(mockOpenAI.chat.completions.create).toHaveBeenCalledWith(expect.objectContaining({ model: "openai/gpt-4o" }), expect.any(Object));
        });
        ...
        it("should throw on AbortError (timeout)", async () => {
            ...
            await expect(translateText("test", "fr")).rejects.toThrow("La traduction a pris trop de temps");
        });
    });
    // ===== summarizeMessages =====
    describe("summarizeMessages", () => {
        ...
        it("should throw on API error", async () => {
            mockOpenAI.chat.completions.create.mockRejectedValueOnce(new Error("API down"));
            await expect(summarizeMessages([{ author: "A", content: "test" }])).rejects.toThrow("Erreur lors du résumé");
        });
        it("should throw on AbortError (timeout)", async () => {
            const abortError = new Error("aborted");
            abortError.name = "AbortError";
            mockOpenAI.chat.completions.create.mockRejectedValueOnce(abortError);
            await expect(summarizeMessages([{ author: "A", content: "test" }])).rejects.toThrow("Le résumé a pris trop de temps");
        });
    });
});
