...
function matchesWishlist(wishlistName, shopName) {
    ...
    if (new RegExp("(^|\\W)" + escaped + "($|\\W)", "i").test(s))
        return true;
    return false;
}
// --- API calls ---
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            for (const item of items) {
                ...
                if (!itemDisplayName)
                    continue; // skip les items sans aucun nom
                ...
                const mapped = {
                    ...
                    rarityColor: getRarityColor(item.rarity?.displayValue || entry.rarity?.displayValue || ""),
                    price: item.price?.finalPrice || item.price?.regularPrice || entry.price?.finalPrice || entry.price?.regularPrice || 0,
                    icon: item.images?.icon || entry.images?.icon || "",
                    featuredImage: item.images?.featured || entry.images?.featured || null,
                    section,
                };
                if (section === "featured")
                    featured.push(mapped);
                else if (section === "daily")
                    daily.push(mapped);
                else if (section.startsWith("specialfeatured"))
                    specialFeatured.push(mapped);
                else if (section.startsWith("specialdaily"))
...
async function checkWishlistMatches(client) {
    ...
    try {
        ...
        for (const entry of allItems) {
            ...
            const namesToCheck = entry.allNames.length > 0 ? entry.allNames : [entry.displayName.toLowerCase().trim()];
        ...
        for (const match of matches) {
            try {
                ...
                const embed = new discord_js_1.EmbedBuilder()
                    ...
                    .addFields({ name: "Rareté", value: match.entry.rarity || "Inconnue", inline: true }, { name: "Prix", value: match.entry.price ? match.entry.price + " V-Bucks" : "Gratuit", inline: true }, { name: "Type", value: match.entry.type || "Cosmétique", inline: true })
                    .setTimestamp();
                if (match.entry.featuredImage)
                    embed.setImage(match.entry.featuredImage);
                else if (match.entry.icon)
...
//# sourceMappingURL=fortnite-api.js.map