...
describe("startMonitoring", () => {
    ...
    it("should log activation message on start", async () => {
        ...
        expect(infoCalls.some((l) => l.includes("Surveillance activée"))).toBe(true);
        expect(infoCalls.some((l) => l.includes("15 min"))).toBe(true);
    });
    ...
    it("should not crash when checkAndNotify encounters internal errors", async () => {
        ...
        expect(errorCalls.some((e) => e.includes("Erreur globale"))).toBe(true);
        setIntervalSpy.mockRestore();
    });
    ...
    it("should call checkAndNotify again when the interval fires", async () => {
        ...
        prisma.source.findMany.mockClear();
...
describe("stopMonitoring", () => {
    ...
    it("should log deactivation message", async () => {
        ...
        expect(logger.info).toHaveBeenCalled();
        const infoCalls = logger.info.mock.calls.map((c) => String(c[0]));
        expect(infoCalls.some((l) => l.includes("Surveillance arrêtée"))).toBe(true);
    });
...
describe("runDbSourcesRetrospective", () => {
    ...
    it("should log start and end markers", async () => {
        ...
        expect(logger.info).toHaveBeenCalled();
        const infoCalls = logger.info.mock.calls.map((c) => String(c[0]));
        expect(infoCalls.some((l) => l.includes("RETROSPECTIVE DB"))).toBe(true);
        expect(infoCalls.some((l) => l.includes("Rattrapage DB terminé"))).toBe(true);
    });
    ...
    it("should stop processing when MAX_RETRO_POSTS cap is reached", async () => {
        ...
        expect(logger.info).toHaveBeenCalled();
        const infoCalls = logger.info.mock.calls.map((c) => String(c[0]));
        expect(infoCalls.some((l) => l.includes("Cap global atteint"))).toBe(true);
        expect(infoCalls.some((l) => l.includes("(2 publications)"))).toBe(true);
