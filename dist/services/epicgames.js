...
function matchesWishlist(wishlistName, shopName) {
    ...
    const boundaryEnd = "($|\\W)";
    if (new RegExp(boundaryStart + escaped + boundaryEnd, "i").test(s))
        return true;
    return false;
}
export async function fetchFreeGames(client) {
    try {
        ...
        for (const el of elements) {
            ...
            const hasFreePromo = allPromos.some((po) => po.promotionalOffers?.some((offer) => {
            ...
            const offerImage = keyImages.find((i) => i.type === "OfferImageWide") ||
                keyImages.find((i) => i.type === "DieselStoreFrontWide") ||
                keyImages[0];
            const imageUrl = offerImage?.url || "";
            const priceInfo = el.price?.totalPrice?.fmtPrice;
            const originalPrice = priceInfo?.originalPrice && priceInfo.originalPrice !== "0"
                ? priceInfo.originalPrice
                : null;
            let freeEndDate = null;
            const firstPromo = allPromos[0]?.promotionalOffers?.[0];
            if (firstPromo?.endDate)
                freeEndDate = firstPromo.endDate;
            games.push({
                title,
                description,
                url,
        ...
        if (newGames.length > 0) {
            ...
            for (const game of newGames) {
                for (const item of wishlistItems) {
                    if (!item.itemName || !matchesWishlist(item.itemName, game.title))
                        continue;
                    ...
                    if (item.guildId)
                        matchMap.get(key).guildIds.add(item.guildId);
                }
            }
            if (matchMap.size > 0) {
...
//# sourceMappingURL=epicgames.js.map