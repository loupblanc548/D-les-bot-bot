...
export async function pruneOldData() {
    ...
    if (logsDeleted > 0 || notificationsDeleted > 0 || chatHistoryDeleted > 0) {
        logger.info(`[DataPruning] Nettoyage terminé : ${logsDeleted} logs, ${notificationsDeleted} notifications, ${chatHistoryDeleted} messages IA`);
    }
    return { logsDeleted, notificationsDeleted, chatHistoryDeleted };
}
let pruneInterval = null;
export function startDataPruning() {
    if (pruneInterval)
        return;
...
export function stopDataPruning() {
    if (pruneInterval) {
