...
export const metrics = {
    incrementCommand: (command, guildId) => {
        commandCounter.inc({ command, guild_id: guildId || "unknown" });
    },
    incrementMessage: (guildId, channelId) => {
    ...
    incrementApiRequest: (service, method, status) => {
        apiRequestCounter.inc({ service, method, status });
    },
    recordApiRequest: (service, duration) => {
        apiRequestDuration.observe({ service }, duration);
    },
    incrementError: (type, module) => {
        errorCounter.inc({ type, module });
    },
    incrementRateLimit: (guildId, type) => {
 ...
 */
export function startMetricsServer(port = parseInt(process.env.METRICS_PORT || "3005")) {
    if (metricsServer)
        return;
