...
function mi(opts = {}) {
    ...
    return {
        ...
        deferReply: vi.fn().mockImplementation(function () { this.deferred = true; return Promise.resolve(); }),
...
describe("handleCommand – vocal", () => {
    ...
    describe("/vocal rejoindre", () => {
        ...
        it("rejoint le salon vocal et confirme avec un embed", async () => {
            ...
            await handleCommand(interaction);
            expect(mockVoice.joinVoiceChannel).toHaveBeenCalledWith(expect.objectContaining({
                channelId: "vc-222",
                guildId: "guild-789",
                selfDeaf: false,
                selfMute: false,
        ...
        it("détruit l'ancienne connexion si le bot est déjà dans un autre salon", async () => {
            ...
            expect(mockVoice.joinVoiceChannel).toHaveBeenCalledWith(expect.objectContaining({ channelId: "vc-222" }));
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining("Connexion précédente détruite"));
        });
