...
exports.commands = exports.CATEGORIES = void 0;
exports.handleCommand = handleCommand;
exports.handleSelectMenu = handleCategorySelect;
...
exports.CATEGORIES = [
    {
        ...
        commands: "`/start - Initialise le bot`\n" +
            "`/help - Cette aide`\n" +
    ...
    },
    {
        ...
        commands: "`/addsource [@handle] [plateforme] - Ajoute une source (YouTube/Twitter/Bluesky)`\n" +
    ...
    },
    {
        ...
        commands: "`/broadcast [message] - Message à tous (admin)`\n" +
    ...
    },
    {
        ...
        commands: "`/chat [message] - Discute avec l'IA`\n" +
    ...
    },
    {
        ...
        commands: "`/alertcenter - Vue d'ensemble des alertes`\n" +
    ...
    },
    {
        ...
        commands: "`/ban [@user] - Bannir (admin)`\n" +
            "`/kick [@user] - Expulser`\n" +
    ...
    },
    {
        ...
        commands: "`/lockdown - Verrouille/déverrouille tous les salons`\n" +
    ...
    },
    {
        ...
        commands: "`/game-status [jeu] - Statut des serveurs de jeu`\n" +
    ...
    },
    {
        ...
        commands: "`/reminder [temps] [message] - Définit un rappel`\n" +
    ...
    },
    {
        ...
        commands: "`/embed-builder - Crée un embed personnalisé`\n" +
    ...
    },
    {
        ...
        commands: "`/casier [@user] - Affiche le casier d'un membre`\n" +
    ...
    },
    {
        ...
        commands: "`/echo-tds - Fait lire un message à haute voix (cooldown 30s)`\n" +
...
async function handleStart(interaction, client) {
    ...
    try {
        const sourcesCount = (await prisma_1.default.source.count()) || 0;
        await interaction.editReply({
            content: "🟢 **Bot opérationnel**\n" +
                "• Version : **1.0.0**\n" +
                "• Latence : **" + client.ws.ping + "ms**\n" +
                "• Sources : **" + sourcesCount + "** surveillée(s)\n" +
                "• Services : Discord.js + Prisma + OpenRouter IA\n" +
                "• " + (config_1.config.adminRoles.length > 0 ? "🟢 Rôles admin configurés" : "🟡 Rôles admin non configurés"),
        });
    }
    catch (error) {
        logger_1.default.error("[CRASH COMMANDE START]:", error);
        try {
...
async function handleHelp(interaction) {
    await interaction.deferReply({ ephemeral: true });
    try {
        const categoryOptions = exports.CATEGORIES.map((cat) => ({
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .addFields({
            name: "📊 Statistiques",
            value: `**${exports.CATEGORIES.length} catégories** • **${exports.CATEGORIES.reduce((acc, cat) => acc + cat.commands.split("\n").length, 0)} commandes**`,
            inline: false,
        })
            .setFooter(FOOTER)
            .setTimestamp();
        ...
        logger_1.default.error("[CRASH COMMANDE HELP]:", error);
        try {
...
async function handleCategorySelect(interaction) {
    ...
    const category = exports.CATEGORIES.find((cat) => cat.id === categoryId);
    if (!category) {
    ...
    const selectMenu = new discord_js_1.StringSelectMenuBuilder()
        .setCustomId("help_category_select")
        .setPlaceholder("Sélectionnez une catégorie...")
        .addOptions(exports.CATEGORIES.map((cat) => ({
        label: `${cat.emoji} ${cat.name}`,
        description: cat.description,
        value: cat.id,
    })));
...
async function handleStatus(interaction, client) {
    ...
    try {
        ...
        const logsCount = await prisma_1.default.log.count();
        const warningsCount = await prisma_1.default.sanction.count({ where: { type: "WARN" } });
        const lastLogs = await (0, logs_1.getLogs)(5);
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .setColor(0x53fc18)
            .addFields({ name: "🟢 Statut", value: "En ligne", inline: true }, { name: "📡 Latence", value: client.ws.ping + "ms", inline: true }, { name: "⏰ Uptime", value: uptimeStr, inline: true }, { name: "📅 Sources", value: sourcesCount.toString(), inline: true }, { name: "📋 Logs", value: logsCount.toString(), inline: true }, { name: "⚠️ Warns", value: warningsCount.toString(), inline: true }, { name: "📝 Dernières actions", value: lastScans, inline: false })
            .setFooter(FOOTER)
            .setTimestamp();
        await interaction.editReply({ embeds: [embed] });
    }
    catch (error) {
        logger_1.default.error("[CRASH COMMANDE STATUS]:", error);
        try {
...
async function handleRestart(interaction, _client) {
    if (!(await (0, permissions_1.requireAdmin)(interaction)))
        return;
    await interaction.deferReply({ ephemeral: true });
    try {
        logger_1.default.info("Redémarrage demandé par", interaction.user.tag);
        await interaction.editReply({ content: "🔄 Redémarrage du bot en cours..." });
        setTimeout(() => { process.exit(0); }, 1000);
    }
    catch (error) {
        logger_1.default.error("[CRASH COMMANDE RESTART]:", error);
        try {
...
async function handleRetro(interaction, client) {
    if (!(await (0, permissions_1.requireAdmin)(interaction)))
        return;
    await interaction.deferReply({ flags: [discord_js_1.MessageFlags.Ephemeral] });
    const yesterday = new Date();
    yesterday.setDate(yesterday.getDate() - 1);
    const recentLogs = await prisma_1.default.log.findMany({
    ...
    const sources = await prisma_1.default.source.count();
    const notifications = await prisma_1.default.notification.count({ where: { sentAt: { gte: yesterday } } });
    const embed = new discord_js_1.EmbedBuilder()
        .setTitle("📊 Rétrospective 24h")
        .setColor(0xffaa00)
        .setDescription("• Du " + yesterday.toLocaleString() + " à maintenant")
        .addFields({ name: "👋 Arrivées", value: memberJoins.toString(), inline: true }, { name: "🚪 Départs", value: memberLeaves.toString(), inline: true }, { name: "🔨 Bans", value: bans.toString(), inline: true }, { name: "🗑️ Msg supprimés", value: messagesDeleted.toString(), inline: true }, { name: "📡 Sources", value: sources + " (" + notifications + " notifs)", inline: true }, { name: "📋 Actions", value: recentLogs.length.toString(), inline: true })
        .setFooter(FOOTER)
        .setTimestamp();
    await interaction.editReply({ embeds: [embed] });
    await interaction.followUp({ content: "🔄 Rattrapage des actualités en cours... Patientez.", flags: [discord_js_1.MessageFlags.Ephemeral] });
    try {
        ...
        logger_1.default.error("[Retro] Erreur lors de la rétrospective manuelle:", String(err));
        await interaction.followUp({
...
async function handleCommand(interaction, client) {
    ...
    switch (commandName) {
        ...
        default:
            logger_1.default.warn(`Commande main inconnue: /${commandName}`);
