...
function mi(o = {}) {
    return {
        options: { getString: vi.fn((n) => n === "action" ? (o.action ?? "add") : (o.nom ?? null)) },
...
describe("add", () => {
    ...
    it("ajoute un item (lowercase+trim)", async () => {
        validateCosmeticName.mockResolvedValue(true);
    ...
    it("refuse item invalide", async () => {
        validateCosmeticName.mockResolvedValue(false);
...
describe("list", () => {
    beforeEach(() => {
        vi.clearAllMocks();
        fetchShop.mockResolvedValue(null); // pas de shop → liste simple
    });
...
describe("autocomplete", () => {
    ...
    it("retourne suggestions (fallback searchCosmetics)", async () => {
        fetchShop.mockResolvedValue(null); // shop down, fallback actif
        searchCosmetics.mockResolvedValue(["A", "B"]);
        const ai = { commandName: "wishlist", options: { getFocused: vi.fn().mockReturnValue({ name: "nom", value: "x" }) }, respond: vi.fn() };
    ...
    it("ignore autre commande", async () => {
        const ai = { commandName: "autre", options: { getFocused: vi.fn() }, respond: vi.fn() };
        await handleAutocomplete(ai);
        expect(ai.respond).not.toHaveBeenCalled();
    });
    it("retourne vide si erreur", async () => {
        fetchShop.mockResolvedValue(null);
        searchCosmetics.mockRejectedValue(new Error("x"));
        const ai = { commandName: "wishlist", options: { getFocused: vi.fn().mockReturnValue({ name: "nom", value: "x" }) }, respond: vi.fn() };
        await handleAutocomplete(ai);
