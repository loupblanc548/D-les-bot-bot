...
describe("buildEntries", () => {
    ...
    it("should group warnings under a WARN header", () => {
        ...
        expect(entries.find((e) => e.isHeader)?.headerLine).toContain("Avertissements");
        expect(entries.some((e) => !e.isHeader && e.line.includes("Spam en chat"))).toBe(true);
    });
    it("should group mutes under a TIMEOUT header", () => {
        ...
        expect(entries.find((e) => e.isHeader)?.headerLine).toContain("Exclusions temporaires");
        expect(entries.some((e) => !e.isHeader && e.line.includes("60 min"))).toBe(true);
    });
    it("should group kicks under a KICK header", () => {
        ...
        expect(entries.find((e) => e.isHeader)?.headerLine).toContain("Expulsions");
        expect(entries.some((e) => !e.isHeader && e.line.includes("Insultes"))).toBe(true);
    });
    it("should include ban sanctions under a BAN header", () => {
        ...
        expect(entries.find((e) => e.isHeader)?.headerLine).toContain("Bannissements");
    });
    it("should include log-based bans", () => {
        ...
        expect(entries.some((e) => !e.isHeader && e.line.includes("BAN"))).toBe(true);
    });
    it("should include the sanction ID in entries", () => {
        const warns = [{ id: 42, type: "WARN", reason: "Test", moderatorId: "mod-1", createdAt: new Date() }];
        const entries = buildEntries(warns, [], [], [], []);
        expect(entries.some((e) => !e.isHeader && e.line.includes("#1"))).toBe(true);
    });
    it("should include moderator mention in entries", () => {
        ...
        const entries = buildEntries(warns, [], [], [], []);
        expect(entries.some((e) => !e.isHeader && e.line.includes("mod-99"))).toBe(true);
    });
    it("should format duration in minutes", () => {
        ...
        const entries = buildEntries([], mutes, [], [], []);
        expect(entries.some((e) => !e.isHeader && e.line.includes("120 min"))).toBe(true);
    });
    it("should handle multiple sections in order", () => {
        ...
        expect(headerTexts.some((h) => h.includes("Avertissements"))).toBe(true);
        expect(headerTexts.some((h) => h.includes("Exclusions temporaires"))).toBe(true);
        expect(headerTexts.some((h) => h.includes("Expulsions"))).toBe(true);
        expect(headerTexts.some((h) => h.includes("Bannissements"))).toBe(true);
    });
});
...
describe("handleCasierClear", () => {
    ...
    it("should reply error when called without id or member", async () => {
        ...
        expect(interaction.reply).toHaveBeenCalledWith(expect.objectContaining({ content: expect.stringContaining("Fournis un ID de sanction") }));
    });
    it("should delete a sanction by ID and reply success", async () => {
        ...
        expect(interaction.reply).toHaveBeenCalledWith(expect.objectContaining({ content: expect.stringContaining("Sanction #5") }));
    });
    it("should reply error when sanction ID not found", async () => {
        ...
        expect(interaction.reply).toHaveBeenCalledWith(expect.objectContaining({ content: expect.stringContaining("introuvable") }));
    });
    it("should delete all sanctions for a member", async () => {
        ...
        interaction.options.getInteger.mockReturnValue(null);
        await handleCasierClear(interaction);
        expect(mockPrisma.sanction.deleteMany).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ userId: "user-99" }) }));
        expect(interaction.reply).toHaveBeenCalledWith(expect.objectContaining({ content: expect.stringContaining("3 sanction(s)") }));
    });
});
describe("handleCommand", () => {
    ...
    it("should error when no guildId", async () => {
        ...
        interaction.commandName = "casier";
        await handleCommand(interaction);
        expect(interaction.reply).toHaveBeenCalledWith(expect.objectContaining({ content: expect.stringContaining("serveur") }));
    });
});
