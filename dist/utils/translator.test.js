...
import { checkCircuitBreaker, banMyMemory, getCircuitBreakerState, resetCircuitBreaker, translateText, } from './translator.js';
// ─── Logger Mock ─────────────────────────────────────────────────────────────
...
function mockFetchSuccess() {
    let callCount = 0;
    global.fetch = vi.fn().mockImplementation(async (_url, _opts) => {
        callCount++;
        if (callCount === 1) {
            // MyMemory
            return {
...
describe('Circuit Breaker — Fallback OpenRouter', () => {
    ...
    it('ne contacte pas MyMemory quand le Circuit Breaker est fermé (banni)', async () => {
        ...
        global.fetch = vi.fn().mockImplementation(async (url, _opts) => {
