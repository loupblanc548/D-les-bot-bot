 ...
 */
export function canSendAlert(key, severity = "medium") {
    const now = Date.now();
 ...
 */
export function forceAlert(key) {
    const entry = cooldownMap.get(key);
    if (entry) {
...
export function enableCooldownCleanup(intervalMs = 300000) {
    if (cleanupInterval) {
 ...
 */
export function disableCooldownCleanup() {
    if (cleanupInterval) {
