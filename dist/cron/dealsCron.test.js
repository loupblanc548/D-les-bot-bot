...
import { checkDeals, startDealsMonitoring, stopDealsMonitoring, detectPlatforms, PLATFORM_CONFIGS, } from "./dealsCron.js";
// ─── Helpers ───────────────────────────────────────────────────────────────
...
describe("checkDeals", () => {
    ...
    describe("Echec fetch RSS", () => {
        it("garde l'erreur d'un flux et continue avec les autres", async () => {
            ...
            await checkDeals(client);
            expect(mockLoggerError).toHaveBeenCalledWith(expect.stringContaining("Erreur analyse du flux"), expect.any(Object));
        });
    });
    ...
    describe("Deduplication via ProcessedDeal", () => {
        ...
        it("gere une erreur Prisma dans isDealProcessed", async () => {
            ...
            expect(mockLoggerWarn).toHaveBeenCalledWith(expect.stringContaining("Erreur verification ProcessedDeal"));
            expect(channel.send).toHaveBeenCalledTimes(1);
        });
    });
    ...
    describe("Fallback plateforme inconnue", () => {
        it("envoie vers le salon PC par defaut si plateforme non detectee", async () => {
            ...
            mockProcessedDealCreate.mockResolvedValue({ id: 1 });
            await checkDeals(client);
            expect(mockLoggerWarn).toHaveBeenCalledWith(expect.stringContaining("Plateforme non detectee"));
            expect(pcChannel.send).toHaveBeenCalledTimes(1);
        });
    });
    ...
    describe("Echec d'envoi", () => {
        it("log l'erreur si channel.send echoue mais continue", async () => {
            ...
            mockProcessedDealCreate.mockResolvedValue({ id: 1 });
            await checkDeals(client);
            expect(mockLoggerError).toHaveBeenCalledWith(expect.stringContaining("Erreur envoi deal"), expect.any(Object));
        });
    });
...
describe("startDealsMonitoring / stopDealsMonitoring", () => {
    it("demarre et arrete la surveillance", () => {
        ...
        expect(mockLoggerInfo).toHaveBeenCalledWith(expect.stringContaining("Demarrage de la surveillance"));
        stopDealsMonitoring();
    ...
    it("empeche le double demarrage", () => {
        ...
        startDealsMonitoring(client);
        startDealsMonitoring(client);
        expect(mockLoggerWarn).toHaveBeenCalledWith(expect.stringContaining("Surveillance deja active"));
    });
