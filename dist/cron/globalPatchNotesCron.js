...
import { isNewItem, markAsProcessed, ContentType, isWithinTemporalBarrier, } from "../managers/ScraperManager.js";
...
async function processPatchNote(client, item) {
    ...
    try {
        ...
        logger.info(`[GlobalPatchNotes] RoutÃ©: "${translatedTitle.slice(0, 60)}" â ${routingResult.sentTo.length} salon(s), ${routingResult.errors.length} erreur(s)`);
        ...
        logger.error(`[GlobalPatchNotes] Ãchec routage: ${error instanceof Error ? error.message : String(error)}`, { stack: error instanceof Error ? error.stack : undefined });
...
export function startGlobalPatchNotesMonitoring(client) {
    if (cronJob) {
...
export function stopGlobalPatchNotesMonitoring() {
    if (cronJob) {
        cronJob.stop();
        cronJob = null;
