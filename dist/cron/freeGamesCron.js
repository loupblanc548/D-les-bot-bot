...
import { ContentType, isNewItem, markAsProcessed, isWithinTemporalBarrier, } from "../managers/ScraperManager.js";
import { FreeGameFetcher } from "../services/FreeGameFetcher.js";
...
async function processFreeGame(client, item) {
    ...
    try {
        const routingResult = await routeArticle(client, translatedTitle, translatedContent, item.link, item.pubDate, imageUrl);
        logger.info(`[FreeGamesCron] Route: "${translatedTitle.slice(0, 60)}" -> ${routingResult.sentTo.length} salon(s), ${routingResult.errors.length} erreur(s)`);
        ...
        logger.error(`[FreeGamesCron] Echec routage: ${error instanceof Error ? error.message : String(error)}`, { stack: error instanceof Error ? error.stack : undefined });
    }
}
// ─── Fetch & Orchestration ─────────────────────────────────────────────────────
...
export function startFreeGamesMonitoring(client) {
    if (cronJob) {
...
export function stopFreeGamesMonitoring() {
    if (cronJob) {
        cronJob.stop();
        cronJob = null;
