...
vi.mock("../utils/translator", () => ({
    ...
    translateToFrench: vi.fn().mockImplementation((text) => Promise.resolve(text)),
}));
...
import { checkTwitterAccounts, startTwitterMonitoring, stopTwitterMonitoring, extractTweetId, } from "./twitterCron.js";
// === Helpers ===
...
describe("checkTwitterAccounts", () => {
    it("r\u00E9cup\u00E8re et posté les tweets de tous les comptes configur\u00E9s", async () => {
        ...
        expect(mockPrisma.processedTweets.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it("continue malgr\u00E9 l'\u00E9chec d'un flux RSS (Promise.allSettled)", async () => {
        ...
        // L'erreur est logg\u00E9e
        expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("Flux RSS inaccessible pour @compte2: Network error"));
...
describe("startTwitterMonitoring / stopTwitterMonitoring", () => {
    it("démarre un check immédiat et crée un intervalle", () => {
        const ch = createMockTextChannel();
        const cl = createMockClient(ch);
        const spy = vi.spyOn(cron, "schedule").mockReturnValue({ stop: vi.fn() });
        startTwitterMonitoring(cl);
        expect(spy).toHaveBeenCalledTimes(1);
        expect(spy).toHaveBeenCalledWith("*/15 * * * *", expect.any(Function));
        expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining("Exécution Cron planifiée"));
    });
    ...
    it("stop supprime l'intervalle proprement", () => {
        ...
        vi.spyOn(cron, "schedule").mockReturnValue({ stop: mockStop });
