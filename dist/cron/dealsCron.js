...
async function sendDealEmbed(client, item, platform) {
    ...
    try {
        ...
        logger.error(`[DealsCron] Erreur envoi deal ${platform.name}: ${error instanceof Error ? error.message : String(error)}`, { stack: error instanceof Error ? error.stack : undefined });
    }
}
async function processDealItem(client, item) {
    ...
    try {
        ...
        for (const platform of platforms) {
            if (!platform.channelId) {
                logger.warn(`[DealsCron] Salon non configure pour ${platform.name}, deal ignore: ${item.title}`);
                continue;
            }
        ...
        logger.error(`[DealsCron] Erreur traitement deal "${item.title}": ${error instanceof Error ? error.message : String(error)}`, { stack: error instanceof Error ? error.stack : undefined });
    }
}
async function checkDeals(client) {
    ...
    if (!hasAnyChannel) {
        logger.warn("[DealsCron] Aucun CHANNEL_ID configure (STEAM_EPIC_CHANNEL_ID, PLAYSTATION_CHANNEL_ID, XBOX_CHANNEL_ID, NINTENDO_CHANNEL_ID) — cron desactive");
    ...
    try {
        for (const feedUrl of RSS_FEEDS) {
            try {
                ...
                logger.error(`[DealsCron] Erreur analyse du flux ${feedUrl}: ${error instanceof Error ? error.message : String(error)}`, { stack: error instanceof Error ? error.stack : undefined });
            }
        }
        metricsCollector.recordProcessing(jobName, true, Date.now() - startTime);
    }
    catch (error) {
        logger.error(`[DealsCron] Erreur globale du cron: ${error instanceof Error ? error.message : String(error)}`, { stack: error instanceof Error ? error.stack : undefined });
        metricsCollector.recordProcessing(jobName, false, Date.now() - startTime);
    }
}
export function startDealsMonitoring(client) {
    if (dealsCronJob) {
    ...
    checkDeals(client).catch((err) => logger.error(`[DealsCron] Erreur check initial: ${err instanceof Error ? err.message : String(err)}`, { stack: err instanceof Error ? err.stack : undefined }));
    dealsCronJob = cron.schedule("*/30 * * * *", () => {
        checkDeals(client).catch((err) => logger.error(`[DealsCron] Erreur check periodique: ${err instanceof Error ? err.message : String(err)}`, { stack: err instanceof Error ? err.stack : undefined }));
    });
}
export function stopDealsMonitoring() {
    if (dealsCronJob) {
        dealsCronJob.stop();
        dealsCronJob = null;
