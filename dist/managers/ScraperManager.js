...
export function getContentTypeConfig(type) {
    ...
    if (!config)
        throw new Error("[ScraperManager] Type de contenu inconnu: " + type);
    return config;
}
/**
 * Verifie si la date de publication est dans la barriere temporelle (24h).
 */
export function isWithinTemporalBarrier(pubDate) {
    if (!pubDate)
        return false;
 ...
 */
export async function executeScraper(options) {
    ...
    if (mode === "rss") {
        try {
            ...
            return { success: false, error: "RSS fetch failed: " + error.message, title: "", content: "", pubDate: "", link: url, image: "" };
        }
    }
    let page = null;
    try {
        ...
        if (sel.content) {
            try {
                content = await page.$$eval(sel.content, (els) => els.map((el) => el.textContent?.trim() || "").join(" "));
            }
            catch { }
        }
        ...
        if (sel.date) {
            try {
                pubDate = await page.$eval(sel.date, (el) => el.getAttribute("datetime") || el.getAttribute("content") || el.textContent?.trim() || "");
            }
            catch { }
        }
        ...
        if (sel.image) {
            try {
                image = await page.$eval(sel.image, (el) => el.getAttribute("src") || el.getAttribute("content") || "");
            }
            catch { }
        }
        image = image.trim();
...
export async function isNewItem(type, uniqueId) {
    ...
    try {
        ...
        logger.error(`[ScraperManager] Erreur dÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ©duplication ${config.tableName}[${config.uniqueField}=${uniqueId}]: ${error}`);
 ...
 */
export async function markAsProcessed(type, uniqueId) {
    const config = getContentTypeConfig(type);
    try {
        const prismaAny = prisma;
        const model = prismaAny[config.tableName];
        ...
        logger.debug(`[ScraperManager] ${config.tableName}[${config.uniqueField}=${uniqueId}] marquÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ© comme traitÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ©`);
    }
    catch (error) {
        // Ignorer uniquement les doublons (contrainte unique P2002)
        if (error?.code === "P2002") {
            logger.debug(`[ScraperManager] ${config.tableName}[${config.uniqueField}=${uniqueId}] dÃÂÃÂÃÂÃÂ©jÃÂÃÂÃÂÃÂ  existant (P2002)`);
            return;
        }
        logger.error(`[ScraperManager] Erreur critique markAsProcessed ${config.tableName}: ${error instanceof Error ? error.message : String(error)}`);
 ...
 */
export async function runScrapingPipeline(url, guid, options, type = ContentType.PATCH_NOTE) {
    const config = getContentTypeConfig(type);
    logger.info(`[ScraperManager] Pipeline [${type}] dÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ©marrÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ©: GUID=${guid} URL=${url}`);
    ...
    try {
        ...
        const errMsg = `Scraping ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ©chouÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ©: ${error.message}`;
 ...
 */
export async function scrapeRssFeed(url, timeout) {
