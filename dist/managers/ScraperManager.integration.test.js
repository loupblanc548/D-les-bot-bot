...
describe('runScrapingPipeline — ContentType paramétrique', () => {
    ...
    it('utilise FREE_GAME quand le type est spécifié', async () => {
        ...
        const promise = runScrapingPipeline('https://example.com/free-game', 'reddit-post-42', undefined, ContentType.FREE_GAME);
        emitSuccessJson(proc);
        const result = await promise;
        expect(result.valid).toBe(true);
    ...
    it('utilise TWEET pour le ContentType correspondant', async () => {
        ...
        const promise = runScrapingPipeline('https://twitter.com/user/status/123', 'tweet-123', { mode: 'html', timeout: 15000 }, ContentType.TWEET);
        emitSuccessJson(proc);
        const result = await promise;
        expect(result.valid).toBe(true);
    ...
    it('passe les options de scraping au spawn Python', async () => {
        ...
        const promise = runScrapingPipeline('https://deals.example.com', 'deal-xyz', { mode: 'html', timeout: 20000 }, ContentType.DEAL);
        emitSuccessJson(proc);
        const result = await promise;
        expect(result.valid).toBe(true);
...
describe('runScrapingPipeline — Timeout', () => {
    ...
    it("retourne skippedReason='scraping_failed' après timeout", async () => {
        const proc = createMockProc();
        mockSpawn.mockReturnValue(proc);
        const promise = runScrapingPipeline('https://slow-server.com', 'guid-timeout', { timeout: 5000 });
