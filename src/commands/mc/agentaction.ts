...
export default {
  ...
  build: (sc) =>
    sc
      ...
      .addStringOption((o) =>
        o
          .setName("action")
          .setDescription("Action: collectWood, collectStone, collectIron, buildHouse, eat, sleep, defend, hunt, stop, explore, sortInventory")
          .setRequired(true)
          .addChoices(
  ...
  execute: async (interaction) => {
    ...
    if (!isAgentAvailable()) {
      await interaction.reply({ content: "❌ Agent non disponible", flags: [MessageFlags.Ephemeral] });
      return;
    }
