...
async function handleNowPlaying(interaction: ChatInputCommandInteraction) {
  ...
  let profiles: Awaited<ReturnType<typeof prisma.steamProfile.findMany>>;

  if (cached && cacheNow < cached.expiry) {
    members = (cached as any).data.members;
    profiles = (cached as any).data.profiles;
  } else {
  ...
  const playingNow: { member: string; game: string; steamId: string }[] = [];
  const idle: string[] = [];
...
async function handleWishlist(interaction: ChatInputCommandInteraction) {
