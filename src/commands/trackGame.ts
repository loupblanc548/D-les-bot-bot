...
async function handleTrackGame(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: "Score de correspondance", value: `${match.score}/1000`, inline: true },
        {
          name: "Dernière news",
          value: latestNews
...
async function handleUntrackGame(interaction: ChatInputCommandInteraction) {
  const gameName = interaction.options.getString("jeu", true).trim();
  await interaction.deferReply({ flags: [MessageFlags.Ephemeral] });
...
async function handleListTracked(interaction: ChatInputCommandInteraction) {
  await interaction.deferReply({ flags: [MessageFlags.Ephemeral] });
