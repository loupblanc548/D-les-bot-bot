...
const PLUS_TIERS: Record<number, string> = {
  0: "Aucun",
...
// ===== /psn trophies =====
async function handleTrophies(interaction: ChatInputCommandInteraction) {
  const username = interaction.options.getString("pseudo", true);
...
function buildProfileEmbed(profile: PsnProfile): EmbedBuilder {
  ...
  return new EmbedBuilder()
    ...
    .setThumbnail(profile.avatarUrl)
    .addFields(
      { name: "🏆 Niveau trophées", value: `${profile.trophySummary.level}`, inline: true },
      { name: "📊 Total trophées", value: `${profile.trophySummary.total}`, inline: true },
