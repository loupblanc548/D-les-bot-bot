...
async function handleGameStatus(interaction: ChatInputCommandInteraction) {
  ...
  const statuses: Record<string, { emoji: string; name: string; url: string; statusUrl: string }> =
    {
      fortnite: {
        ...
        statusUrl: "https://status.epicgames.com/api/v2/status.json",
      },
      epic: {
        emoji: "🎮",
      ...
      roblox: {
        ...
        statusUrl: "https://status.roblox.com/api/v2/status.json",
      },
      nintendo: {
        emoji: "🎮",
        name: "Nintendo",
        url: "https://www.nintendo.com/fr-fr/",
        statusUrl: "https://www.nintendo.com/fr-fr/",
      },
      steam: {
        emoji: "🎮",
        name: "Steam",
        url: "https://steamstat.us/",
        statusUrl: "https://crowbar.steamstat.us/",
      },
      psn: {
        emoji: "🎮",
      ...
      xbox: {
        emoji: "🎮",
        name: "Xbox Live",
        url: "https://support.xbox.com/fr-FR/xbox-live-status",
        statusUrl: "https://support.xbox.com/fr-FR/xbox-live-status",
      },
      ea: {
        emoji: "🎮",
        name: "EA App",
        url: "https://www.ea.com/fr-fr/ea-app",
        statusUrl: "https://www.ea.com/fr-fr/ea-app",
      },
      ubisoft: {
        emoji: "🎮",
        name: "Ubisoft Connect",
        url: "https://www.ubisoft.com/fr-fr/",
        statusUrl: "https://www.ubisoft.com/fr-fr/",
      },
      riot: {
        emoji: "🎮",
        name: "Riot Games",
      ...
      helldivers2: {
        emoji: "🎮",
        name: "Helldivers 2",
        url: "https://www.playstation.com/fr-fr/games/helldivers-2/",
        statusUrl: "https://www.playstation.com/fr-fr/games/helldivers-2/",
      },
      gta: {
        emoji: "🎮",
        name: "GTA Online",
        url: "https://support.rockstargames.com/fr/",
        statusUrl: "https://support.rockstargames.com/fr/",
      },
      cod: {
        emoji: "🎮",
        name: "Call of Duty",
        url: "https://support.activision.com/fr/call-of-duty",
        statusUrl: "https://support.activision.com/fr/call-of-duty",
      },
    };
  ...
  if (!game) {
    await interaction.editReply({
      embeds: [
        new EmbedBuilder()
          .setDescription("❌ Jeu/service non reconnu.")
          .setColor(0xff3344)
          .setFooter(FOOTER)
          .setTimestamp(),
      ],
    ...
    await interaction.editReply({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(desc)
          .setColor(color)
          .addFields({ name: "🔗 Page statut", value: game.url, inline: false })
          .setFooter(FOOTER)
          .setTimestamp(),
      ],
    });
...
async function handleFreeGames(interaction: ChatInputCommandInteraction) {
  ...
  try {
    const res = await fetch(
      "https://store-site-backend-static-ipv4.ak.epicgames.com/freeGamesPromotions?locale=fr-FR&country=FR&allowCountries=FR",
      ...
      await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              "• Aucun jeu gratuit disponible pour le moment.\n• Consulte le [Epic Games Store](https://store.epicgames.com/fr/free-games) !",
            )
            .setColor(0xffaa00)
            .setFooter(FOOTER)
            .setTimestamp(),
    ...
    const endDate = game?.promotions?.promotionalOffers?.[0]?.promotionalOffers?.[0]?.endDate
      ...
      : "Limite";
    const embed = new EmbedBuilder()
      .setAuthor({ name: "JEUX GRATUITS DU MOMENT" })
      .setTitle("🎮 " + title + " — GRATUIT")
      .setDescription(desc.slice(0, 1024) || "Fonce le récupérer !")
      .setColor(0x00f0ff)
      .addFields(
        { name: "💰 Prix original", value: originalPrice, inline: true },
        { name: "⏰ Fin de l'offre", value: endDate, inline: true },
        {
          name: "🔗 Lien",
          value: "[Epic Games Store](https://store.epicgames.com/fr/free-games)",
          inline: true,
        },
      )
      .setFooter(FOOTER)
    ...
    if (freeGames.length > 1)
      embed.addFields({
        name: "📦 Autres",
        value: freeGames.length - 1 + " autre(s) jeu(x) gratuit(s)",
        inline: true,
      });
    await interaction.editReply({ embeds: [embed] });
  } catch (_err) {
    await interaction.editReply({
      embeds: [
        new EmbedBuilder()
          .setTitle("🎮 Jeux gratuits")
          .setDescription(
            "• Impossible de récupérer les jeux gratuits.\n• Consulte [Epic Games Store](https://store.epicgames.com/fr/free-games)",
          )
          .setColor(0xff3344)
          .setFooter(FOOTER)
          .setTimestamp(),
...
async function handlePatchNotes(interaction: ChatInputCommandInteraction) {
  ...
  if (!game) {
    await interaction.editReply({
      embeds: [
        new EmbedBuilder()
          .setDescription("❌ Jeu non reconnu.")
          .setColor(0xff3344)
          .setFooter(FOOTER)
          .setTimestamp(),
      ],
  ...
  await interaction.editReply({
    embeds: [
      new EmbedBuilder()
        .setTitle(game.emoji + " Patch Notes — " + game.name)
        .setDescription(
          "• Consulte les derniers patch notes de **" + game.name + "** :\n• " + game.url,
        )
        .setColor(0x2f3136)
        .setFooter(FOOTER)
        .setTimestamp(),
  ...
  try {
    ...
    if (!result || result.prices.length === 0) {
      const notFound = !result
        ? "• Aucun résultat trouvé pour **" + gameName + "**."
        : "• Jeu trouvé mais aucun prix disponible pour le moment.";
      await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            .setAuthor({ name: "Comparateur de prix" })
            .setTitle("💰 " + gameName)
            .setDescription(
              notFound + "\n" + "• Consultez [IsThereAnyDeal](https://isthereanydeal.com/)",
            )
            .setColor(0xffaa00)
            .setFooter(FOOTER)
            .setTimestamp(),
