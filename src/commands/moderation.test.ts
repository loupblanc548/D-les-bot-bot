...
function createMockInteraction(overrides = {}) {
  return {
    ...
    options: {
      getUser: vi
        .fn()
        .mockReturnValue({ id: "target-123", tag: "Target#1234", username: "Target" }),
...
describe("handleCommand — 16 commandes", () => {
  ...
  it("/lock gère les erreurs de permission avec fallback", async () => {
    const interaction = createMockInteraction({ commandName: "lock" });
    interaction.channel.permissionOverwrites.edit = vi
      .fn()
    ...
    const textChannel = {
      ...
      messages: {
        fetch: vi
          .fn()
          .mockResolvedValue(
            new Map([["m-1", { id: "m-1", author: { id: "target-123" }, delete: vi.fn() }]]),
          ),
  ...
  it("/tempban bannit temporairement avec format durée valide (1h)", async () => {
    ...
    await handleCommand(interaction, {});
    expect(interaction.guild.members.ban).toHaveBeenCalledWith(mockTarget, {
      reason: "test reason",
