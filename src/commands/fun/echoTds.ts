...
function getCooldownRemaining(userId: string): number {
  ...
  if (elapsed >= COOLDOWN_MS) { cooldowns.delete(userId); return 0; }
...
export const commands = [
  new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName("message").setDescription("Le texte que le bot doit lire à haute voix").setRequired(true).setMaxLength(1000)
    ).toJSON(),
];
...
function scheduleCooldownCleanup() {
  ...
  cleanupInterval = setInterval(() => {
    const now = Date.now();
    for (const [userId, timestamp] of cooldowns.entries()) { if (now - timestamp >= COOLDOWN_MS) cooldowns.delete(userId); }
    if (cooldowns.size === 0 && cleanupInterval) { clearInterval(cleanupInterval); cleanupInterval = null; }
  }, 5 * 60 * 1000);
}
