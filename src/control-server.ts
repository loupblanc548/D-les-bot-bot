 ...
 * src/bot.ts line ~72.
 */
export async function startControlServer(_port: number, _client: unknown): Promise<void> {
  logger.warn(
    "[control-server] STUB: HTTP control server not implemented; port=blank, /health and /metrics endpoints will not respond.",
  );
...
export async function stopControlServer(): Promise<void> {
  logger.warn("[control-server] STUB: stopControlServer invoked but not implemented.");
