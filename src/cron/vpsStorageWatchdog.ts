 ...
 *
 * Checks disk utilization every 30 minutes.
 *  - >80%: Yellow alert
 *  - >90%: Critical alert → Layer 4 Validation Gate DM
 *
...
import { checkVpsStorage, startUptimeHeartbeat, logRecoveryState } from "../services/vpsMaintenance.js";
...
export function startVpsStorageWatchdog(): void {
  ...
  void checkVpsStorage().catch((err) => {
    logger.debug(`[VPS-STORAGE-WATCHDOG] Initial check failed: ${err instanceof Error ? err.message : String(err)}`);
  });

  // Schedule recurring disk check (every 30 min)
  storageCron = schedule("*/30 * * * *", () => {
    void checkVpsStorage().catch((err) => {
      logger.debug(`[VPS-STORAGE-WATCHDOG] Check failed: ${err instanceof Error ? err.message : String(err)}`);
    });
  });
