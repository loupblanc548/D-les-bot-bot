...
describe("extractImage", () => {
  ...
  it("Tier 4: builds Steam banner from AppID in link", async () => {
    ...
    const r = await extractImage(item, undefined, {});
    expect(r).toBe(
      "https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/123456/header.jpg",
    );
  });
  ...
  it("Tier 5: skipped for deal aggregators (Instant Gaming)", async () => {
    ...
    const rc = makeRawgClient({
      byTitle: { "Some Random Deal": "https://cdn.rawg.example/x.jpg" },
    });
    const r = await extractImage(
      item,
      { name: "Instant Gaming", channelEnv: "INSTANT_GAMING_CHANNEL_ID" },
      { rawgClient: rc },
    );
    expect(r).toBeNull();
