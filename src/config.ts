...
export const config = {
  ...
  controlPort: parseInt(process.env.CONTROL_PORT || "3002", 10),
  ...
  // Database
  databaseUrl: process.env.DATABASE_URL || "postgresql://discord_bot:discord_bot@localhost:5432/discord_bot?schema=public",
...
export function validateConfig(): { errors: string[]; warnings: string[] } {
  ...
  if (!config.logChannel) warnings.push("LOG_CHANNEL_ID manquant dans .env (les logs ne seront pas envoyes)");
  if (!config.freeGamesChannel) warnings.push("FREE_GAMES_CHANNEL_ID manquant dans .env (les alertes de jeux gratuits seront desactivees)");
  // Owner (warning)
  if (!config.ownerId) warnings.push("OWNER_ID manquant dans .env (notifications owner desactivees)");
  // Gaming APIs (warnings)
  if (!config.steamApiKey) warnings.push("STEAM_API_KEY manquant dans .env (commandes /steam desactivees)");
  if (!config.twitterChannel) warnings.push("TWITTER_CHANNEL_ID manquant dans .env (surveillance Twitter desactivee)");
  if (!config.twitterAccounts) warnings.push("TWITTER_ACCOUNTS manquant dans .env (surveillance Twitter desactivee)");
  if (!config.twitchClientId) warnings.push("TWITCH_CLIENT_ID manquant dans .env (monitoring Twitch desactive)");
  if (!config.twitchClientSecret) warnings.push("TWITCH_CLIENT_SECRET manquant dans .env (monitoring Twitch desactive)");
  // Monitoring (warning)
  if (!config.sentryDsn) warnings.push("SENTRY_DSN manquant dans .env (monitoring Sentry desactive)");
