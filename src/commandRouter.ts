...
import { commands as mainCommands, handleCommand as handleMain, handleSelectMenu as handleMainSelectMenu } from "./commands/main";
...
import { commands as securityCommands, handleCommand as handleSecurity } from "./commands/security/core";
import { commands as gamingCommands, handleCommand as handleGaming } from "./commands/gaming";
import { commands as communityCommands, handleCommand as handleCommunity } from "./commands/community";
...
import { commands as retrospectiveCommands, handleCommand as handleRetrospective } from "./commands/retrospective";
...
import { commands as trackGameCommands, handleCommand as handleTrackGame } from "./commands/trackGame";
import { commands as psnCommands, handleCommand as handlePsn } from "./commands/psn";
import { commands as echoTdsCommands, handleCommand as handleEchoTds } from "./commands/fun/echoTds";
import { commands as askBotCommands, handleCommand as handleAskBot } from "./commands/fun/askBot";
import { commands as wishlistCommands, handleCommand as handleWishlist } from "./commands/fun/wishlist";
...
import { commands as alertcenterCommands, handleCommand as handleAlertcenter } from "./commands/alertcenter";
import { commands as mp3Commands, handleCommand as handleMp3 } from "./commands/mp3";
import { commands as cleanDuplicatesCommands, handleCommand as handleCleanDuplicates } from "./commands/clean-duplicates";
...
export const allCommands = [
  ...mainCommands, ...sourceCommands, ...adminCommands, ...aiCommands, ...modCommands, ...casierCommands,
  ...securityCommands, ...gamingCommands, ...communityCommands, ...utilityCommands, ...vocalCommands,
  ...retrospectiveCommands, ...twitchCommands, ...steamCommands, ...psnCommands, ...trackGameCommands,
  ...echoTdsCommands, ...askBotCommands, ...wishlistCommands, ...shopCommands, ...aiExtraCommands,
  ...dicteeCommands, ...alertcenterCommands, ...mp3Commands, ...cleanDuplicatesCommands,
];

function registerGroup(groupNames: string[], handler: Function /* eslint-disable-line @typescript-eslint/no-unsafe-function-type */): void {
...
export function buildCommandRouter(): void {
  ...
  registerGroup(["ban", "kick", "mute", "unmute", "warn", "clear", "timeout", "lock", "unlock", "softban", "purge", "slowmode", "snipe", "history"], handleModeration);
  registerGroup(["casier", "casier-clear"], handleCasier);
  registerGroup(["lockdown", "nuke", "check-alt", "blacklist", "role-mass", "antiraid", "verif", "namehistory", "avatarhistory", "linkcheck", "antiphishing", "guildconfig"], handleSecurity);
...
export async function registerCommands(): Promise<void> {
  try {
    ...
    if (config.guildId) {
      ...
      logger.info(`✓ ${allCommands.length} commandes enregistrees pour la guilde ${config.guildId}`);
    } else {
    ...
    logger.error(`Erreur d'enregistrement des commandes: ${error instanceof Error ? error.message : String(error)}`, { stack: error instanceof Error ? error.stack : undefined });
  }
}
