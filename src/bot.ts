...
async function main(): Promise<void> {
  ...
  try { startHealthServer(3000); } catch { logger.warn("Health server failed to start (port 3000 in use?)"); }
  try { startMetricsServer(); } catch { logger.warn("Metrics server failed to start (port 3005 in use?)"); }
  startControlServer(config.controlPort, client).catch(() => logger.warn("[Startup] Control server failed to start"));

  // Nettoyage initial + automatique
  pruneOldData().catch((err) => logger.error(`[Pruning] Erreur nettoyage initial: ${err instanceof Error ? err.message : String(err)}`));
  startDataPruning();
  ...
  // Initialiser Sentry
  if (config.sentryDsn) {
    Sentry.init({ dsn: config.sentryDsn, tracesSampleRate: 0.3, environment: process.env.NODE_ENV || "production" });
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        logger.warn("Warm-up cache: " + (warmupError instanceof Error ? warmupError.message : String(warmupError)));
      }
      ...
      logger.warn(`⚠ Cache anti-doublon: ${dedupError instanceof Error ? dedupError.message : String(dedupError)}`);
    }
  } catch (error) {
    logger.error(`❌ Erreur de connexion a la base de donnees: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  }
  ...
  try {
    ...
    logger.error(`❌ Erreur de connexion a Discord: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  }
}
