...
export class TwitchChatBot extends EventEmitter {
  ...
  connect(): void {
    if (!TWITCH_OAUTH_TOKEN) {
      logger.warn("[TwitchChatBot] No TWITCH_OAUTH_TOKEN");
      return;
    }
    ...
    this.ws.on("close", () => {
      ...
      if (this.autoReconnect) setTimeout(() => this.connect(), 5000);
    });
  ...
  private handleMessage(raw: string): void {
    for (const line of raw.split("\r\n").filter(Boolean)) {
      if (line.startsWith("PING")) {
        this.sendRaw("PONG :" + line.split(":")[1]);
        continue;
      }
    ...
    try {
      ...
      if (tagMatch)
        for (const pair of tagMatch[1].split(";")) {
          ...
          tags[safeKey] = safeVal;
        }
      const userMatch = line.match(/:([^!]+)!/);
      const chMatch = line.match(/PRIVMSG #([^ :]+)/);
      ...
      return {
        ...
        timestamp: new Date(Number(tags["tmi-sent-ts"] || Date.now())),
      };
    } catch {
      return null;
    }
  ...
  private handleCommand(msg: TwitchChatMessage): void {
    ...
    if (handler) {
      if (typeof handler !== "function") {
        logger.warn(`[TwitchChatBot] Invalid handler for ${parts[0]}`);
        return;
      }
      Promise.resolve(handler(msg, parts.slice(1))).catch((err) =>
        logger.error(
          `[TwitchChatBot] Cmd error: ${err instanceof Error ? err.message : String(err)}`,
        ),
      );
    }
  }
