 ...
 */
async function gatherContext(_client: Client): Promise<ProactiveContext> {
  let serverActivity = 0;
  let recentTopics: string[] = [];

  try {
    const since = new Date(Date.now() - 30 * 60 * 1000);
    const recentLogs = await prisma.log
      .findMany({
...
async function proactiveThink(client: Client): Promise<void> {
  ...
  const channel = await client.channels.fetch(PROACTIVE_CHANNEL ?? "").catch((): null => null);
  ...
  try {
    ...
    if (trends.length > 0) {
      ...
      if (!alreadyPosted) {
        ...
        await recordDecision({
          type: "trends",
          action: trends[0].title.slice(0, 50),
          success: true,
          context: "auto",
        });
      }
  ...
  if (actions.length > 0) {
    ...
    await recordDecision({
      type: "proactive",
      action: `reflect_${actions.length}_actions`,
      success: true,
...
async function wasRecentAction(
  type: string,
  substring: string,
  ...
  return recent.some(
    (d) =>
      ...
      d.action.toLowerCase().includes(substring.toLowerCase()),
  );
}
...
async function autonomousMonitor(client: Client): Promise<void> {
  ...
  if (memMB > 1800) {
    ...
    await recordDecision({
      type: "gc",
      action: `auto_gc_${memMB.toFixed(0)}mb`,
      success: true,
      context: "auto",
    });
  }
...
export function startProactiveAgent(client: Client): void {
  ...
  monitorCron = cron.schedule("*/10 * * * *", () => {
    autonomousMonitor(client).catch((err) =>
      logger.error(
        `[Proactive] Erreur monitor: ${err instanceof Error ? err.message : String(err)}`,
      ),
    );
  });
