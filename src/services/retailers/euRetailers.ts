...
async function searchAlternate(
  query: string,
  country: CountryCode,
  limit = 10,
  ...
  try {
    ...
    const blocks =
      html.match(/class="listRow[\s\S]*?(?=class="listRow|<div class="pagination)/g) || [];
...
async function searchMindfactory(
  query: string,
