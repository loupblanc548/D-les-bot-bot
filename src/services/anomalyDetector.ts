...
export interface AnomalyReport {
  guildId: string;
  alerts: AnomalyAlert[];
  overallRisk: "normal" | "elevated" | "high" | "critical";
  checkedAt: string;
}
...
export async function detectAnomalies(guildId: string): Promise<AnomalyReport> {
  ...
  const [
    ...
    prisma.chatHistory
      .count({ where: { guildId, createdAt: { gte: twoHoursAgo, lt: oneHourAgo } } })
      .catch(() => 0),
    prisma.errorMessage.count({ where: { createdAt: { gte: oneHourAgo } } }).catch(() => 0),
    prisma.errorMessage
      .count({ where: { createdAt: { gte: twoHoursAgo, lt: oneHourAgo } } })
      .catch(() => 0),
  ...
  if (previousMessages > 0) {
    ...
    if (dev > 200)
      alerts.push({
        type: "message_spike",
        severity: dev > 500 ? "critical" : "high",
        guildId,
        metric: "messages_per_hour",
        currentValue: recentMessages,
        expectedValue: previousMessages,
        deviationPercent: dev,
        description: `Pic d'activité: ${recentMessages} messages/h vs ${previousMessages} précédent (+${dev}%)`,
      ...
      alerts.push({
        ...
        expectedValue: previousMessages,
        deviationPercent: dev,
        description: `Activité élevée: +${dev}% vs heure précédente`,
        timestamp: now.toISOString(),
      });
  }
  ...
  if (previousErrors > 0) {
    ...
    if (dev > 100)
      alerts.push({
        type: "error_spike",
        severity: dev > 300 ? "critical" : "high",
        guildId,
        metric: "errors_per_hour",
        currentValue: recentErrors,
        expectedValue: previousErrors,
        deviationPercent: dev,
        description: `Pic d'erreurs: ${recentErrors} erreurs/h vs ${previousErrors} (+${dev}%)`,
        timestamp: now.toISOString(),
      });
    ...
    alerts.push({
      ...
      description: `${recentErrors} erreurs dans la dernière heure`,
      timestamp: now.toISOString(),
    });
  }
  ...
  if (previousMod > 0) {
    ...
    if (dev > 200)
      alerts.push({
        ...
        severity: "high",
        guildId,
        metric: "mod_actions_per_hour",
        currentValue: recentMod,
        expectedValue: previousMod,
        deviationPercent: dev,
        description: `Pic de modération: ${recentMod} actions/h (+${dev}%)`,
        timestamp: now.toISOString(),
      });
    ...
    alerts.push({
      ...
      expectedValue: 0,
      deviationPercent: 100,
      description: `${recentMod} actions de modération dans la dernière heure`,
      timestamp: now.toISOString(),
    });
  }
  ...
  if (recentJoins > 20) {
    alerts.push({
      type: "new_user_flood",
      severity: recentJoins > 50 ? "critical" : "high",
      guildId,
      metric: "new_users_24h",
      currentValue: recentJoins,
      expectedValue: 5,
      deviationPercent: Math.round((recentJoins / 5) * 100),
      description: `${recentJoins} nouveaux membres en 24h — possible raid`,
      timestamp: now.toISOString(),
    });
  }
  ...
  const overallRisk: AnomalyReport["overallRisk"] =
    criticalCount > 0
      ? "critical"
      : highCount > 1
        ? "high"
...
export function detectSpike(
  values: number[],
  threshold = 2.5,
): { index: number; value: number; expected: number; zScore: number }[] {
