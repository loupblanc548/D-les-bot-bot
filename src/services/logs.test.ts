...
describe("sendErrorLog", () => {
  ...
  it("should log the error to logger.error regardless of config", async () => {
    ...
    const calls = (logger.error as any).mock.calls.map((c: any[]) => c.map(String).join(" "));
    expect(calls.some((c: string) => c.includes("TestModule"))).toBe(true);
  ...
  it("should attempt to send embed when logChannel and client are provided", async () => {
    ...
    sendErrorLog("Database", err, mockClient as any);
  ...
  it("should truncate long error messages to 1024 chars", async () => {
    ...
    sendErrorLog("Test", err, mockClient as any);
  ...
  it("should silently catch errors when channel send fails", async () => {
    ...
    expect(() => sendErrorLog("Module", new Error("test"), mockClient as any)).not.toThrow();
  ...
  it("should skip sending if log channel is not text-based", async () => {
    ...
    sendErrorLog("Module", new Error("test"), mockClient as any);
...
describe("sendBanPurgeLog", () => {
  ...
  it("should send a properly structured embed to the log channel", async () => {
    ...
    sendBanPurgeLog("TestUser#0001", "user-42", 15, 8, mockClient as any);
  ...
  it("should silently catch errors when channel send fails", async () => {
    ...
    await sendBanPurgeLog("User#X", "user-x", 0, 1, mockClient as any);
