    ...
    switch (toolName) {
      ...
      case "derivative_calculator": {
        ...
        try {
          ...
          return ok(
            `Dérivée de ${expr} par rapport à ${variable}. Pour un calcul symbolique complet, utilisez execute_code avec mathjs.`,
          );
        }
      }
      case "integral_calculator": {
        ...
        try {
          ...
          return ok(
            `Intégrale de ${expr}. Pour un calcul symbolique, utilisez execute_code avec mathjs.`,
          );
        }
      }
      ...
      case "complex_number_ops": {
        const a = argNum(args, "real_a"),
          b = argNum(args, "imag_a");
        const c = argNum(args, "real_b"),
          d = argNum(args, "imag_b");
      ...
      case "confidence_interval": {
        const mean = argNum(args, "mean"),
          std = argNum(args, "std_dev"),
          n = argNum(args, "sample_size");
      ...
      case "hypothesis_test": {
        ...
        return ok(
          `p-value=${pValue}, alpha=${alpha}. ${pValue < alpha ? "Rejeter H0 (significatif)" : "Ne pas rejeter H0 (non significatif)"}`,
        );
      }
      case "limit_calculator":
        return ok(
          `Limite de ${arg(args, "expression")} quand ${arg(args, "variable", "x")} → ${arg(args, "approaches", "0")}. Utilisez execute_code avec mathjs.`,
        );
      ...
      case "matrix_operations": {
        ...
        if (!Array.isArray(m1) || !Array.isArray(m2))
          return fail("matrix_a et matrix_b requis (number[][])");
        try {
          if (op === "add") {
          ...
          if (op === "multiply") {
            ...
            for (let i = 0; i < rows; i++)
              for (let j = 0; j < cols; j++)
                for (let k = 0; k < inner; k++) result[i][j] += m1[i][k] * m2[k][j];
            return ok(`A × B =\n${result.map((r) => r.join("\t")).join("\n")}`);
          }
          if (op === "transpose") {
          ...
          return fail(`Erreur matrice: ${e}`);
        }
      }
      case "modular_arithmetic": {
        const a = argNum(args, "a"),
          b = argNum(args, "b"),
          m = argNum(args, "modulus");
        const op = arg(args, "operation", "add");
        let r: number;
      ...
      case "moving_average_calc": {
        ...
        return ok(
          `Moving average (window=${window}): [${result.map((v) => v.toFixed(2)).join(", ")}]`,
        );
      }
      case "outlier_detector": {
        ...
        return ok(
          `Mean=${mean.toFixed(2)}, Std=${std.toFixed(2)}, Outliers (z>2): [${outliers.join(", ")}]`,
        );
      }
      ...
      case "probability_distribution": {
        return ok(
          `Distribution: ${arg(args, "distribution")}. Utilisez execute_code pour calculer.`,
        );
      }
      case "series_sum_calculator": {
        ...
        if (type === "arithmetic") {
          const a = argNum(args, "first"),
            d = argNum(args, "diff");
          return ok(`Sum = ${(n / 2) * (2 * a + (n - 1) * d)}`);
        }
        if (type === "geometric") {
          const a = argNum(args, "first"),
            r = argNum(args, "ratio");
          return ok(`Sum = ${(a * (1 - Math.pow(r, n))) / (1 - r)}`);
        }
      ...
      case "trigonometry_solver": {
        ...
        return ok(
          `sin=${Math.sin(rad).toFixed(6)}, cos=${Math.cos(rad).toFixed(6)}, tan=${Math.tan(rad).toFixed(6)}`,
        );
      }
      case "vector_calculus": {
        ...
        try {
          ...
          if (op === "cross") {
            if (!Array.isArray(v2) || v1.length !== 3 || v2.length !== 3)
              return fail("Cross product nécessite 2 vecteurs 3D");
            const cross = [
              v1[1] * v2[2] - v1[2] * v2[1],
              v1[2] * v2[0] - v1[0] * v2[2],
              v1[0] * v2[1] - v1[1] * v2[0],
      ...
      case "stats_calc": {
        ...
        return ok(
          `Mean=${mean.toFixed(4)}, Median=${median}, Std=${Math.sqrt(variance).toFixed(4)}, Min=${sorted[0]}, Max=${sorted[sorted.length - 1]}, Range=${sorted[sorted.length - 1] - sorted[0]}`,
        );
      }
      ...
      case "radioactive_decay_calc_2": {
        ...
        return ok(
          `Remaining after ${time} units: ${remaining.toFixed(4)} (decay constant λ=${(Math.log(2) / halfLife).toFixed(6)})`,
        );
      }
      case "ideal_gas_law": {
        const p = argNum(args, "pressure"),
          v = argNum(args, "volume"),
          t = argNum(args, "temperature");
        const r = 8.314;
        const solveFor = arg(args, "solve_for", "n");
        if (solveFor === "n")
          return ok(`n = PV/RT = (${p}×${v})/(${r}×${t}) = ${((p * v) / (r * t)).toFixed(6)} mol`);
        if (solveFor === "p")
          return ok(`P = nRT/V = ${((argNum(args, "moles") * r * t) / v).toFixed(6)} Pa`);
        if (solveFor === "v")
          return ok(`V = nRT/P = ${((argNum(args, "moles") * r * t) / p).toFixed(6)} m³`);
        return ok(`T = PV/nR = ${((p * v) / (argNum(args, "moles") * r)).toFixed(6)} K`);
      }
      case "ohms_law_calc": {
        const v = argNum(args, "voltage"),
          i = argNum(args, "current"),
          r = argNum(args, "resistance");
      ...
      case "ph_calculator": {
        ...
        if (oh > 0)
          return ok(
            `pOH = -log₁₀(${oh}) = ${(-Math.log10(oh)).toFixed(4)}, pH = ${14 + Math.log10(oh)}.toFixed(4)`,
          );
        return fail("h_concentration ou oh_concentration requis");
      }
      case "thermal_expansion_calc": {
        const l0 = argNum(args, "initial_length"),
          alpha = argNum(args, "coefficient"),
          dt = argNum(args, "delta_t");
        return ok(`ΔL = α×L₀×ΔT = ${alpha}×${l0}×${dt} = ${(alpha * l0 * dt).toFixed(6)}`);
      }
      case "electric_field_calc": {
        const q = argNum(args, "charge"),
          r = argNum(args, "distance");
        const k = 8.99e9;
        return ok(`E = k×q/r² = ${k}×${q}/${r * r} = ${((k * q) / (r * r)).toExponential(4)} N/C`);
      }
      case "optics_calc": {
        return ok(
          `Calcul optique: ${arg(args, "operation")}. Utilisez execute_code pour des calculs détaillés.`,
        );
      }
      case "kinematics_calc": {
        const v0 = argNum(args, "initial_velocity"),
          a = argNum(args, "acceleration"),
          t = argNum(args, "time");
        return ok(`v = v₀ + at = ${v0 + a * t}\nx = v₀t + ½at² = ${v0 * t + 0.5 * a * t * t}`);
      }
      case "wavelength_frequency": {
        const c = 3e8;
        const f = argNum(args, "frequency"),
          wl = argNum(args, "wavelength");
      ...
      case "molar_mass_calc": {
        ...
        try {
          ...
          return ok(`Masse molaire de ${formula}. Utilisez execute_code pour un calcul détaillé.`);
        }
      }
      ...
      case "astronomical_distance": {
        ...
        if (parallax > 0)
          return ok(
            `Distance = 1/parallax = 1/${parallax} = ${(1 / parallax).toFixed(4)} parsecs = ${(3.26 / parallax).toFixed(4)} light-years`,
          );
        return fail("parallax_mas requis");
      }
      // Health & fitness
      case "body_fat_percentage_calc": {
        const w = argNum(args, "weight"),
          h = argNum(args, "height");
        const age = argNum(args, "age"),
          gender = arg(args, "gender", "male");
        const bmi = w / (h / 100) ** 2;
      ...
      case "heart_rate_zone": {
        ...
        const maxHR = 220 - age;
        return ok(
          `Max HR=${maxHR}, Zone 1(${Math.round(maxHR * 0.5)}-${Math.round(maxHR * 0.6)}), Zone 2(${Math.round(maxHR * 0.6)}-${Math.round(maxHR * 0.7)}), Zone 3(${Math.round(maxHR * 0.7)}-${Math.round(maxHR * 0.8)}), Zone 4(${Math.round(maxHR * 0.8)}-${Math.round(maxHR * 0.9)}), Zone 5(${Math.round(maxHR * 0.9)}-${maxHR})`,
        );
      }
      case "hydration_tracker": {
        const weight = argNum(args, "weight_kg");
        return ok(
          `Recommandation: ${Math.round(weight * 0.033 * 10) / 10}L/jour (${Math.round((weight * 0.033 * 1000) / 250)} verres de 250ml)`,
        );
      }
      case "ideal_weight_calc": {
        const h = argNum(args, "height_cm"),
          gender = arg(args, "gender", "male");
      ...
      case "macro_nutrient_calc": {
        ...
        return ok(
          `Répartition ${calories}kcal: Protéines=${protein}g, Glucides=${carbs}g, Lipides=${fat}g`,
        );
      }
      case "ovulation_calc": {
      ...
      case "pregnancy_due_date": {
        ...
        return ok(
          `Date prévue d'accouchement ≈ ${lmp} + 280 jours. Utilisez execute_code pour calculer la date exacte.`,
        );
      }
      case "sleep_quality_score": {
        const hours = argNum(args, "hours_slept"),
          deep = argNum(args, "deep_sleep_pct", 20);
      ...
      // Encoding / crypto
      case "base_convert":
      case "number_base_convert_advanced":
      case "binary_convert":
      case "hex_convert": {
      ...
      case "generate_hmac": {
        const data = arg(args, "data"),
          algo = arg(args, "algorithm", "sha256");
      ...
      case "frequency_analysis": {
        ...
        const sorted = Object.entries(freq)
          ...
          .slice(0, 10);
        return ok(
          `Top 10: ${sorted.map(([c, n]) => `${c}:${((n / text.length) * 100).toFixed(1)}%`).join(", ")}`,
        );
      }
      default:
        return null;
    ...
    switch (toolName) {
      ...
      case "text_json_to_csv": {
        try {
          ...
          if (!Array.isArray(data)) return fail("JSON doit être un array");
          const headers = Object.keys(data[0]);
          const rows = data.map((o: Record<string, any>) =>
            headers.map((h) => String(o[h] ?? "")).join(","),
          );
          return ok([headers.join(","), ...rows].join("\n"));
        } catch {
          return fail("JSON invalide");
        }
      }
      ...
      case "text_readability_score": {
        ...
        const syllables = words.reduce(
          (s, w) => s + (w.match(/[aeiouyàâéèêëiouôû]/gi) || []).length,
          0,
        );
        const flesch =
          206.835 - 1.015 * (words.length / sentences.length) - 84.6 * (syllables / words.length);
        return ok(
          `Flesch Reading Ease: ${flesch.toFixed(1)} (${flesch > 60 ? "lisible" : "difficile"})`,
        );
      }
      case "text_regex_tester": {
        const pattern = arg(args, "pattern"),
          flags = arg(args, "flags", "g");
        try {
          const re = new RegExp(pattern, flags);
          const matches = text.match(re) || [];
          return ok(`Matches (${matches.length}): ${matches.slice(0, 20).join(", ")}`);
        } catch (e) {
          return fail(`Regex invalide: ${e}`);
        }
      }
      ...
      case "text_transliterate": {
        return ok(
          `Translittération de "${text.slice(0, 50)}...". Utilisez execute_code pour une translittération complète.`,
        );
      }
      ...
      case "text_language_detect_advanced": {
        const lang = /[àâéèêëîïôûùç]/i.test(text)
          ? "Français"
      ...
      case "text_stem_lemmatize": {
        return ok(
          `Stemming/Lemmatisation de "${text.slice(0, 50)}...". Utilisez execute_code avec natural/compromise pour traitement NLP.`,
        );
      }
      ...
      case "csv_analyzer": {
        ...
        const headers = lines[0].split(",").map((h) => h.trim());
        return ok(
          `CSV: ${lines.length - 1} lignes, ${headers.length} colonnes. Headers: ${headers.join(", ")}`,
        );
      }
      ...
      case "regex_debugger": {
        ...
        try {
          new RegExp(pattern);
          return ok(`Regex valide: /${pattern}/`);
        } catch (e) {
          return fail(`Regex invalide: ${e}`);
        }
      }
      ...
      case "code_complexity_analyzer": {
        return ok(
          `Analyse de complexité cyclomatique. Utilisez execute_code avec eslint pour analyser.`,
        );
      }
      case "code_linter_check": {
        return ok(`Lint du code. Utilisez execute_code avec eslint pour vérifier.`);
      }
      case "sql_format_beautifier":
      case "sql_format_beautify_2": {
        return ok(`Formatage SQL. Utilisez execute_code avec sql-formatter pour formater.`);
      }
      case "sql_query_explainer": {
        return ok(
          `Explication de requête SQL: ${arg(args, "query")}. Analysez la requête pour expliquer JOINs, WHERE, GROUP BY, etc.`,
        );
      }
      case "changelog_generator": {
        return ok(
          `Génération de changelog. Utilisez execute_code avec conventional-changelog pour générer.`,
        );
      }
      case "data_anonymizer": {
        const anon = text
          ...
          .replace(/\b[A-Z]{2}\d{9}[A-Z]\b/g, "FRXXXXXXXXX");
        return ok(anon);
      }
      case "text_to_speech_multi": {
        return ok(
          `TTS pour "${text.slice(0, 50)}...". Utilisez l'API AssemblyAI ou Google TTS pour générer l'audio.`,
        );
      }
      default:
        return null;
...
const KALI_TOOLS = new Set([
  ...
  "subdomain_enum",
  "crtsh_search",
  "google_dorks_generator",
  "github_dorks_search",
  "command_injection_test",
  "csrf_token_check",
  "xss_payload_generator",
  "sqli_payload_generator",
  "xxe_vuln_check",
  "lfi_rfi_check",
  "open_redirect_check",
  "ssrf_check",
  ...
  "docker_ps_audit",
  "docker_image_vuln_scan",
  "kubernetes_manifest_validate",
  "terraform_validate",
  "terraform_plan_diff",
  ...
  "env_vars_inspect",
  "cron_jobs_list",
  "load_average_monitor",
  "disk_usage_analyzer",
  "log_tail",
  "memory_leak_detect",
  "network_connections_list",
  "network_map_generate",
  "process_monitor",
  "port_kill",
...
async function handleSecurity(
  ...
  try {
    ...
    const target =
      arg(args, "target") || arg(args, "url") || arg(args, "host") || arg(args, "domain");
    return ok(
      `🔧 ${toolName} sur ${target || "cible"} — utilise le container Kali Linux. Vérifiez que le container Kali est démarré. Arguments: ${JSON.stringify(args).slice(0, 200)}`,
    );
  } catch (e) {
  ...
  try {
    switch (toolName) {
      ...
      case "github_dorks_search": {
        ...
        return ok(
          `GitHub dorks pour "${query}" (${data.items?.length || 0} results):\n${results.join("\n")}`,
        );
      }
      ...
      case "crtsh_search": {
        ...
        const certs = data
          .slice(0, 15)
          .map(
            (c, i) => `${i + 1}. ${c.name_value} (issuer: ${c.issuer_name}, from: ${c.not_before})`,
          );
      ...
      case "wayback_machine_lookup": {
        ...
        if (data.archived_snapshots?.closest)
          return ok(
            `Snapshot: ${data.archived_snapshots.closest.url} (${data.archived_snapshots.closest.timestamp})`,
          );
        return ok(`Aucun snapshot trouvé pour ${url}`);
      }
      case "wayback_diff": {
        const url = arg(args, "url"),
          t1 = arg(args, "timestamp1"),
          t2 = arg(args, "timestamp2");
        return ok(
          `Diff Wayback Machine pour ${url} entre ${t1} et ${t2}. Utilisez: https://timetravel.mementoweb.org/api/`,
        );
      }
      case "google_cache_lookup": {
        const url = arg(args, "url");
        return ok(
          `Google Cache pour ${url}: https://webcache.googleusercontent.com/search?q=cache:${encodeURIComponent(url)}`,
        );
      }
      ...
      case "reverse_image_search": {
        ...
        return ok(
          `Reverse image search: Google Images (https://lens.google.com/uploadbyurl?url=${encodeURIComponent(imageUrl)}) ou TinEye (https://tineye.com/search/?url=${encodeURIComponent(imageUrl)})`,
        );
      }
      case "social_media_checker": {
        const username = arg(args, "username");
        return ok(
          `Vérification username "${username}" sur réseaux sociaux: https://www.namechk.com/ ou utilisez execute_code avec Sherlock.`,
        );
      }
      case "geocode_reverse": {
        const lat = argNum(args, "lat"),
          lon = argNum(args, "lon");
        const res = await fetch(
      ...
      case "distance_matrix": {
        const origins = arg(args, "origins"),
          destinations = arg(args, "destinations");
        return ok(
          `Distance matrix: ${origins} → ${destinations}. Utilisez Google Distance Matrix API ou OpenRouteService.`,
        );
      }
      case "capital_lookup":
      case "country_bordering":
      case "currency_by_country":
      case "iso_country_code":
      case "language_by_country": {
        ...
        if (toolName === "country_bordering")
          return ok(`Frontières: ${c.borders?.join(", ") || "Aucune"}`);
        if (toolName === "currency_by_country")
          return ok(
            `Devise: ${Object.values(c.currencies || {})
              .map((d) => d.name)
              .join(", ")}`,
          );
        if (toolName === "iso_country_code") return ok(`Code ISO: ${c.cca2 || "N/A"}`);
        if (toolName === "language_by_country")
          return ok(`Langues: ${Object.values(c.languages || {}).join(", ")}`);
      ...
      case "fortnite_item_shop": {
        try {
          ...
          const data = (await res.json()) as {
            data?: { entries?: Array<{ items?: Array<{ name: string }>; finalPrice?: number }> };
          };
          const items = (data.data?.entries || [])
            .slice(0, 10)
            .map(
              (e, i) =>
                `${i + 1}. ${e.items?.[0]?.name || "Unknown"} — ${e.finalPrice || "?"} V-Bucks`,
            );
          return ok(
            `🛒 Boutique Fortnite (${data.data?.entries?.length || 0} items):\n${items.join("\n")}`,
          );
        } catch {
          return ok("Boutique Fortnite indisponible. Utilisez: https://fortnite-api.com/v2/shop");
        }
      }
      case "epic_games_free_games": {
        try {
          ...
          return ok("Epic Games API indisponible.");
        }
      }
      ...
      case "twitch_stream_check": {
        ...
        return ok(
          `Check Twitch ${channel}: nécessite TWITCH_ACCESS_TOKEN. Utilisez l'API: GET https://api.twitch.tv/helix/streams?user_login=${channel}`,
        );
      }
      case "twitch_clip_create": {
        return ok(
          `Création clip Twitch: nécessite TWITCH_ACCESS_TOKEN et broadcaster_id. POST https://api.twitch.tv/helix/clips`,
        );
      }
      case "spotify_track_search": {
        return ok(
          `Recherche Spotify: nécessite SPOTIFY_CLIENT_ID/SECRET. Utilisez: GET https://api.spotify.com/v1/search?q=${encodeURIComponent(arg(args, "query"))}&type=track`,
        );
      }
      ...
      case "apex_legends_stats": {
        ...
        try {
          const res = await fetch(
            `https://api.mozambiquehe.re/bridge?auth=${process.env.APEX_API_KEY || "free"}&player=${encodeURIComponent(player)}&platform=PC`,
            { signal: AbortSignal.timeout(8000) },
          );
          const data = (await res.json()) as {
            global?: { name: string; level: number; rank?: { rankName: string } };
          };
          if (data.global)
            return ok(
              `🎮 Apex Legends — ${data.global.name}: Niveau ${data.global.level}, Rank: ${data.global.rank?.rankName || "N/A"}`,
            );
          return ok(`Joueur "${player}" introuvable sur Apex Legends.`);
        } catch {
          return ok(
            `Apex Legends stats pour "${player}". Nécessite APEX_API_KEY (gratuit sur https://apexlegendsapi.com).`,
          );
        }
      }
      case "csgo_stats_fetch": {
        const player = arg(args, "player");
        return ok(
          `CS2 stats pour "${player}". Utilisez: https://tracker.gg/csgo/profile/steam/${encodeURIComponent(player)}/overview`,
        );
      }
      case "lol_match_history":
      case "lol_rank_check":
      case "riot_account_lookup": {
        const player = arg(args, "player");
        try {
          const res = await fetch(
            `https://euw1.api.riotgames.com/lol/summoner/v4/summoners/by-name/${encodeURIComponent(player)}`,
            {
          ...
          return ok(
            `LoL stats pour "${player}". Nécessite RIOT_API_KEY (gratuit sur https://developer.riotgames.com).`,
          );
        }
      }
      case "osu_user_stats": {
        ...
        try {
          ...
          const data = (await res.json()) as {
            username?: string;
            statistics?: { global_rank?: number; pp?: number };
          };
          if (data.username)
            return ok(
              `🎵 osu! — ${data.username}: Rank #${data.statistics?.global_rank || "N/A"}, PP: ${data.statistics?.pp || 0}`,
            );
          return ok(`Joueur osu! "${player}" introuvable.`);
        } catch {
          return ok(
            `osu! stats pour "${player}". Nécessite OSU_API_KEY (gratuit sur https://osu.ppy.sh/home/account/edit).`,
          );
        }
      }
      case "rocket_league_stats": {
        const player = arg(args, "player");
        return ok(
          `🚀 Rocket League stats pour "${player}". Utilisez: https://rocketleague.tracker.network/rocket-league/profile/epic/${encodeURIComponent(player)}/overview`,
        );
      }
      ...
      case "hunter_io_email":
      case "leaked_source_search":
      case "breach_parse":
      case "malware_sample_lookup":
      ...
      case "bitcoin_address_analysis": {
        ...
        return ok(
          `BTC ${addr}: Balance=${data.balance / 1e8} BTC, Total reçu=${data.total_received / 1e8} BTC, Transactions=${data.n_tx}`,
        );
      }
      case "ethereum_contract_verify": {
        ...
        return ok(
          `ETH contract ${addr}: ${data.status === "1" ? "Vérifié, ABI disponible" : "Non vérifié ou source non disponible"}`,
        );
      }
      case "get_dev_snippet":
      case "lookup_typescript_skill":
      case "search_developer_resources":
      case "search_programming_books":
      case "search_public_apis":
      case "search_system_design": {
        const query = arg(args, "query");
        return ok(
          `${toolName}: "${query}". Utilisez searchWeb ou execute_code pour des résultats détaillés.`,
        );
      }
      default:
        return null;
  ...
  try {
    switch (toolName) {
      case "rsa_keypair_generate": {
        ...
        return ok(
          `Génération paire RSA ${bits} bits. Utilisez execute_code: crypto.generateKeyPairSync('rsa', { modulusLength: ${bits} })`,
        );
      }
      case "rsa_encrypt":
      case "rsa_decrypt": {
        return ok(
          `${toolName}: nécessite clé PEM. Utilisez execute_code avec crypto.publicEncrypt/crypto.privateDecrypt.`,
        );
      }
      case "pgp_encrypt":
      case "pgp_decrypt": {
        return ok(
          `${toolName}: nécessite openpgp. Utilisez execute_code avec la librairie openpgp.`,
        );
      }
      case "crypto_aes_decrypt": {
        return ok(
          `AES decrypt: nécessite clé + IV. Utilisez execute_code: crypto.createDecipheriv('aes-256-gcm', key, iv)`,
        );
      }
      case "certificate_parse": {
        ...
        try {
          ...
          return ok(
            parsed
              ? `Subject: ${parsed.subject}, Issuer: ${parsed.issuer}, Valid: ${parsed.validFrom} - ${parsed.validTo}`
              : "Parsing non disponible",
          );
        } catch {
          return fail("Certificat invalide");
        }
      }
      case "stego_hide_lsb":
      case "stego_extract_lsb":
      case "steganalysis_zscore": {
        return ok(
          `${toolName}: stéganographie LSB. Utilisez execute_code avec jimp/sharp pour traiter l'image.`,
        );
      }
      case "metadata_strip":
      case "image_metadata_strip":
      case "exif_extract_full": {
      ...
      case "histogram_generator":
      case "scatter_plot_generator": {
        return ok(`${toolName}: génération de graphique. Utilisez execute_code avec chart.js/d3.`);
      }
      case "time_series_decompose": {
        return ok(
          `Décomposition série temporelle. Utilisez execute_code avec statsmodels/simple-statistics.`,
        );
      }
      case "timezone_convert_advanced": {
        const dt = arg(args, "datetime"),
          fromTz = arg(args, "from_timezone"),
          toTz = arg(args, "to_timezone");
        try {
          const d = new Date(dt);
          return ok(
            `${dt} (${fromTz}) → ${d.toLocaleString("fr-FR", { timeZone: toTz })} (${toTz})`,
          );
        } catch {
          return fail("Datetime/timezone invalide");
        }
      }
      ...
      case "unit_convert_scientific": {
        return ok(
          `Conversion scientifique: ${arg(args, "value")} ${arg(args, "from_unit")} → ${arg(args, "to_unit")}. Utilisez execute_code avec convert-units.`,
        );
      }
      case "rate_limit_check": {
        return ok(
          `Rate limit check pour ${arg(args, "endpoint")}. Mesurez les appels API récents.`,
        );
      }
      ...
      case "api_rate_limit_discover": {
        ...
        return ok(
          `Rate limits pour ${url}: limit=${rateLimit}, retry-after=${headers["retry-after"] || "N/A"}`,
        );
      }
      case "api_schema_diff": {
        return ok(
          `Diff de schema API entre ${arg(args, "schema1")} et ${arg(args, "schema2")}. Utilisez execute_code pour comparer.`,
        );
      }
      ...
      case "dep_vuln_check": {
        ...
        return ok(
          `${pkg}@${data.version}. Vérifiez https://www.npmjs.com/advisories pour vulnérabilités.`,
        );
      }
      default:
        return null;
