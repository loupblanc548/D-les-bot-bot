  ...
  try {
    ...
    if (guildConfig?.logChannelId) {
      ...
      if (guild) {
        ...
        if (channel?.isTextBased()) targetChannel = channel as TextChannel;
      }
    }
  } catch (err) {
  ...
  for (const ownerId of ownerIds) {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          { name: "Serveur", value: alert.guildId, inline: true },
          { name: "Utilisateur", value: `<@${alert.userId}>`, inline: true },
          { name: "Score", value: `${alert.riskScore}`, inline: true },
        )
        .setTimestamp();
...
// ============================================================
export async function getPendingAlerts(guildId: string) {
