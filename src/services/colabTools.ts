...
const COLAB_TOOLS_ENABLED = process.env.COLAB_TOOLS_URL !== undefined || process.env.LLM_DYNAMIC_URL === "true";
...
export async function classifyNsfwViaColab(imageUrl: string): Promise<NsfwResult | null> {
  ...
  try {
    ...
    logger.info(`[ColabTools] NSFW result: score=${result?.nsfw_score?.toFixed(2)} nsfw=${result?.is_nsfw}`);
...
export async function detectAiImageViaColab(imageUrl: string): Promise<AiDetectResult | null> {
  ...
  try {
    ...
    logger.info(`[ColabTools] AI detect: score=${result?.ai_score?.toFixed(2)} ai=${result?.is_ai}`);
...
export async function transcribeViaColab(audioUrl: string): Promise<TranscribeResult | null> {
  ...
  try {
    ...
    logger.info(`[ColabTools] Transcribed: ${result?.text?.length} chars, lang=${result?.language}`);
...
export async function embeddingsViaColab(text: string): Promise<EmbeddingsResult | null> {
  ...
  try {
    ...
    logger.info(`[ColabTools] Embeddings: ${result?.embedding?.length} dims, model=${result?.model}`);
