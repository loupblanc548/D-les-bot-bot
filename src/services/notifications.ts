...
export function isSlackConfigured(): boolean {
  return SLACK_WEBHOOK_URL.length > 0;
}
...
export async function sendTelegramMessage(text: string): Promise<boolean> {
  ...
  try {
    ...
    logger.error(`[Notify] Telegram: ${err instanceof Error ? err.message : String(err)}`);
    return false;
  }
}
...
export async function sendSlackMessage(text: string, blocks?: any[]): Promise<boolean> {
  ...
  try {
    ...
    logger.error(`[Notify] Slack: ${err instanceof Error ? err.message : String(err)}`);
    return false;
  }
}
...
export async function sendDiscordWebhook(message: {
  ...
  try {
    ...
    logger.error(`[Notify] Discord webhook: ${err instanceof Error ? err.message : String(err)}`);
    return false;
  }
}
...
export async function broadcastNotification(
  text: string,
): Promise<{ telegram: boolean; slack: boolean; discord: boolean }> {
