import { Client, MessageReaction, User, TextChannel, Role, PartialMessageReaction, PartialUser } from "discord.js";
...
export function startReactionRoles(client: Client): void {
  client.on("messageReactionAdd", async (reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser) => {
    try {
      ...
      logger.error(`[ReactionRoles] Erreur add: ${err instanceof Error ? err.message : String(err)}`);
    }
  });
  ...
  client.on("messageReactionRemove", async (reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser) => {
    try {
      ...
      logger.error(`[ReactionRoles] Erreur remove: ${err instanceof Error ? err.message : String(err)}`);
    }
  });
