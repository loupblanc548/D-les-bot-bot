...
export function getOmnirouteClient(): OpenAI | null {
  ...
  if (!omnirouteClient) {
    omnirouteClient = new OpenAI({
      ...
      timeout: 15_000,
      // Fail over at the model-rotation layer instead of hiding retries here.
      maxRetries: 0,
    });
  }
...
/**
 * Check if a model is one of the exact OmniRoute models we expose.
 * Prefix checks are intentionally avoided: names such as `z-ai/*` can also
 * be valid OpenRouter models and must not be sent to the wrong endpoint.
 */
export function isOmnirouteModel(modelName: string): boolean {
  return isOmnirouteAvailable() && OMNIROUTE_FREE_MODELS.includes(modelName);
}
...
export async function chatWithOmniroute(opts: ChatOptions): Promise<string | null> {
  ...
  try {
    ...
    logger.debug(
      `[OmniRoute] Chat failed: ${error instanceof Error ? error.message : String(error)}`,
    );
    return null;
  }
}
