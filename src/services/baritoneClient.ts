...
const BARITONE_ENABLED = process.env.BARITONE_URL !== undefined || process.env.BARITONE_DYNAMIC_URL === "true";
...
export async function sendBaritoneCommand(command: string): Promise<{ success: boolean; message: string }> {
  const url = getUrl();
  if (!url) {
  ...
  try {
    ...
    return { success: false, message: `❌ Erreur: ${err instanceof Error ? err.message : String(err)}` };
  }
}
...
export async function sendMinecraftChat(message: string): Promise<{ success: boolean; message: string }> {
  const url = getUrl();
  if (!url) return { success: false, message: "❌ Baritone backend non disponible" };
  try {
...
/** Go to coordinates. */
export async function baritoneGoto(x: number, z: number): Promise<{ success: boolean; message: string }> {
  return sendBaritoneCommand(`goto ${x} ${z}`);
}

/** Mine a specific ore/block. */
export async function baritoneMine(oreName: string): Promise<{ success: boolean; message: string }> {
  return sendBaritoneCommand(`mine ${oreName}`);
}

/** Follow a player. */
export async function baritoneFollow(playerName: string): Promise<{ success: boolean; message: string }> {
...
export async function baritoneBuild(structure: string): Promise<{ success: boolean; message: string }> {
