...
const PLATFORM_CHANNELS: Record<string, { channelId: string; keywords: string[]; label: string; color: number }> = {
  steam: { channelId: config.steamEpicChannel, keywords: ["steam", "valve", "cs2", "csgo", "dota", "tf2", "gmod", "pc gaming"], label: "Steam/Epic", color: 0x1b2838 },
  playstation: { channelId: config.playstationChannel, keywords: ["playstation", "ps5", "ps4", "sony", "spider-man", "god of war", "horizon"], label: "PlayStation", color: 0x003791 },
  xbox: { channelId: config.xboxChannel, keywords: ["xbox", "game pass", "microsoft", "halo", "forza", "fable", "sega"], label: "Xbox", color: 0x107c10 },
  nintendo: { channelId: config.nintendoChannel, keywords: ["nintendo", "switch", "mario", "zelda", "pokemon", "smash", "metroid"], label: "Nintendo", color: 0xe60012 },
  fortnite: { channelId: config.fortniteChannel, keywords: ["fortnite", "epic", "battle royale", "zero build", "creative"], label: "Fortnite", color: 0x9147ff },
  deals: { channelId: config.dealsChannel, keywords: ["deal", "promo", "soldes", "discount", "reduction", "gratuit", "free"], label: "Deals", color: 0xffaa00 },
};
...
async function analyzeChannelMessages(channel: TextChannel, platform: string, keywords: string[]): Promise<TopicStat[]> {
  ...
  try {
    ...
    logger.debug(`[HotTopics] Erreur analyse ${channel.name}: ${err instanceof Error ? err.message : String(err)}`);
  }
  ...
  for (const [platformKey, platformCfg] of Object.entries(PLATFORM_CHANNELS)) {
    ...
    for (const topic of topTopics) {
      embed.addFields({
        ...
        value: topic.sampleMessages.map((s) => `> ${s}`).join("\n").substring(0, 1024) || "—",
        inline: false,
      });
    }
    ...
    try {
      ...
      logger.info(`[HotTopics] Rapport envoyé pour ${platformCfg.label} — ${topTopics.length} sujet(s) chaud(s)`);
    } catch (err) {
...
export function startHotTopicsDetector(client: Client): void {
  ...
  logger.info("[HotTopics] Détecteur de sujets chauds activé (intervalle: 1h) — routing par plateforme");
