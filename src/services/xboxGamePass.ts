...
export async function fetchXboxGamePassGames(): Promise<XboxGamePassGame[]> {
  try {
    ...
    const response = await fetch(
      "https://catalog.gamepass.com/sigls/v2?id=2b8ac61a-e65e-4f01-b428-5bda35b4d0c1&language=en&market=US",
    );
    if (!response.ok) {
    ...
    if (data && Array.isArray(data)) {
      for (const id of data) {
        const gameResponse = await fetch(
          `https://displaycatalog.mp.microsoft.com/v3.0/products?bigIds=${id}&market=US&languages=en-us&MS-CV=DGU`,
        );
        if (gameResponse.ok) {
