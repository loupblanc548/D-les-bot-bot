...
const resolveTxt = promisify(dns.resolveTxt) as (hostname: string) => Promise<string[][]>;
const resolveMx = promisify(dns.resolveMx) as (
  hostname: string,
) => Promise<{ priority: number; exchange: string }[]>;
const resolveNs = promisify(dns.resolveNs) as (hostname: string) => Promise<string[]>;
const resolveCname = promisify(dns.resolveCname) as (hostname: string) => Promise<string[]>;
const resolveA = promisify(dns.resolve4) as (hostname: string) => Promise<string[]>;
const resolveAaaa = promisify(dns.resolve6) as (hostname: string) => Promise<string[]>;
const reverseDns = promisify(dns.reverse) as (ip: string) => Promise<string[]>;
...
export async function fullDnsLookup(domain: string): Promise<FullDnsLookup> {
  ...
  const [a, aaaa, mx, ns, txt, cname] = await Promise.all([
    resolveA(d).catch((): string[] => []),
    resolveAaaa(d).catch((): string[] => []),
    resolveMx(d).catch((): { priority: number; exchange: string }[] => []),
    resolveNs(d).catch((): string[] => []),
    resolveTxt(d).catch((): string[][] => []),
    resolveCname(d).catch((): string[] => []),
  ]);
  result.a = a as string[];
  result.aaaa = aaaa as string[];
  ...
  for (const r of result.mx)
    result.records.push({ type: "MX", value: `${r.priority} ${r.exchange}` });
  ...
  for (const ip of result.a.slice(0, 3)) {
    try {
      const ptr = await reverseDns(ip).catch((): string[] => []);
      result.ptr.push(...(ptr as string[]));
    } catch {
      /* skip */
    }
  }
  ...
  try {
    ...
    return {
      ...
      nameServers: Array.isArray(raw.nameServers)
        ? raw.nameServers.map(String)
        : String(raw.nameServers || "")
            .split(/\s+/)
            .filter(Boolean),
      status: Array.isArray(raw.status)
        ? raw.status.map(String)
        : [String(raw.status || "")].filter(Boolean),
...
export async function checkEmailSecurity(
  domain: string,
): Promise<{ spf: boolean; dmarc: boolean; dkim: boolean }> {
  const d = domain
    .replace(/^https?:\/\//, "")
    .replace(/\/.*$/, "")
    .toLowerCase();
  const txt = await resolveTxt(d).catch((): string[][] => []);
  const dmarc = await resolveTxt(`_dmarc.${d}`).catch((): string[][] => []);
  ...
  for (const sel of ["default", "google", "selector1", "s1"]) {
    const r = await resolveTxt(`${sel}._domainkey.${d}`).catch((): string[][] => []);
    if (r.length > 0) {
      dkim = true;
      break;
    }
...
export async function assessDomainRisk(
  domain: string,
  ...
  if (w?.creationDate) {
    ...
    if (ageDays < 30) {
      score += 30;
      reasons.push(`Domain registered ${Math.round(ageDays)} days ago`);
    } else if (ageDays < 90) {
      score += 15;
      reasons.push(`Domain registered ${Math.round(ageDays)} days ago`);
    }
  ...
  if (suspiciousTlds.some((t) => domain.toLowerCase().endsWith(t))) {
    score += 20;
