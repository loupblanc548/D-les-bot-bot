...
export interface TranslationResult {
  ...
  confidence: number;
  provider: "google" | "libre" | "none";
}

export async function translateAny(
  text: string,
  targetLang = "fr",
  sourceLang?: string,
  ...
  try {
    ...
    if (res.data?.translatedText) {
      return {
        ...
        detectedSourceLanguage: String(
          res.data.detectedLanguage?.language || sourceLang || "unknown",
        ),
        targetLanguage: targetLang,
        confidence: 0.8,
        provider: "libre",
    ...
    logger.error(`[LibreTranslate] ${err instanceof Error ? err.message : String(err)}`);
    return null;
  }
}

export async function detectLanguageAuto(
  text: string,
): Promise<{ language: string; confidence: number; provider: string }> {
  const googleResult = await googleDetect(text).catch((): null => null);
  if (googleResult && googleResult.confidence > 0)
    return {
  ...
  try {
    ...
    if (Array.isArray(res.data) && res.data.length > 0) {
      return {
        language: String(res.data[0].language || "unknown"),
        confidence: Number(res.data[0].confidence || 0),
        provider: "libre",
      };
    }
  } catch {
...
export async function autoTranslateIfNeeded(
  ...
  return {
    translated: result.provider !== "none",
    original: text,
...
export function isLibreTranslateConfigured(): boolean {
  return LIBRETRANSLATE_URL.length > 0;
}
