import { Client, GuildMember, TextChannel, EmbedBuilder, Role } from "discord.js";
...
export function startOnboardingFlow(client: Client): void {
  ...
  client.on("guildMemberAdd", async (member: GuildMember) => {
    try {
      ...
      logger.error(`[Onboarding] Erreur welcome: ${err instanceof Error ? err.message : String(err)}`);
    }
  });

  logger.info("[Onboarding] Flow d'onboarding activé (welcome + reaction roles)");
  onboardingInterval = safeInterval("Onboarding", () => {
