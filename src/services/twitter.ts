...
export function isTwitterConfigured(): boolean {
  return TWITTER_BEARER_TOKEN.length > 0;
}
...
export async function getUser(username: string): Promise<TwitterUser | null> {
  ...
  try {
    ...
    return {
      ...
      profileImageUrl: String(d.profile_image_url || ""),
      createdAt: String(d.created_at || ""),
    };
  } catch (err) {
    logger.error(`[Twitter] getUser: ${err instanceof Error ? err.message : String(err)}`);
    return null;
  }
}
...
export async function getRecentTweets(userId: string, max = 5): Promise<TwitterTweet[]> {
  ...
  try {
    ...
    logger.error(`[Twitter] getRecentTweets: ${err instanceof Error ? err.message : String(err)}`);
    return [];
  }
}
...
export async function searchTweets(query: string, max = 5): Promise<TwitterTweet[]> {
  ...
  try {
    ...
    logger.error(`[Twitter] searchTweets: ${err instanceof Error ? err.message : String(err)}`);
    return [];
  }
}
