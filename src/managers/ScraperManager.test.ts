...
const _mockFindUnique = vi.hoisted(() => vi.fn());
const _mockCreate = vi.hoisted(() => vi.fn());
...
function getMockForType(type: ContentType) {
  const map: Record<ContentType, typeof mockProcessedTweets> = {
...
const EXPECTED_CONFIGS: Record<ContentType, { tableName: string; uniqueField: string }> = {
...
describe("markAsProcessed â Marquage gÃ©nÃ©rique (7 ContentTypes)", () => {
  ...
  for (const type of ALL_CONTENT_TYPES) {
    ...
    describe(`ContentType.${type.toUpperCase()}`, () => {
      ...
      it("ne lance pas d'erreur si le modèle Prisma est introuvable (catch silencieux)", async () => {
        const type = ContentType.TWEET;
        const mock = getMockForType(type);
        mock.create.mockRejectedValue(new Error("Modèle introuvable"));
        await expect(markAsProcessed(type, "no-model-id")).resolves.toBeUndefined();
      });
