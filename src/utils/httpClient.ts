 ...
 */
export function createCircuitBreaker<T extends (...args: any[]) => Promise<any>>(fn: T, opts?: {
  ...
  return {
    ...
    forceOpen: (ms?: number) => { state = "OPEN"; openUntil = Date.now() + (ms ?? cooldownMs); },
    forceClose: () => { state = "CLOSED"; failures = []; },
  };
}
