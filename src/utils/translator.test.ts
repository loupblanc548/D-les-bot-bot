...
function mockFetchSuccess(): void {
  let callCount = 0;
  global.fetch = vi.fn().mockImplementation(async (_url: string | URL | Request, _opts?: RequestInit) => {
    callCount++;
    if (callCount === 1) {
      // MyMemory
      return {
...
describe('Circuit Breaker — Fallback OpenRouter', () => {
  ...
  it('ne contacte pas MyMemory quand le Circuit Breaker est fermé (banni)', async () => {
    ...
    global.fetch = vi.fn().mockImplementation(async (url: string | URL | Request, _opts?: RequestInit) => {
