...
interface EscalationRule {
  ...
  cooldown: number;
  escalateAfter: number; // nombre d'alertes avant escalation
  escalateTo: "low" | "medium" | "high" | "critical";
  notifyRoles?: string[]; // IDs des rôles à notifier
  notifyUsers?: string[]; // IDs des utilisateurs à notifier
}
 ...
 */
export async function sendEscalatedAlert(
  client: Client,
  key: string,
  message: string,
  data?: Record<string, unknown>
  ...
  if (!rule) {
    ...
    return await sendDirectAlert(client, key, message, data as any);
  }
 ...
 */
export function resetAlert(key: string): void {
  alertMap.delete(key);
  resetCooldown(key);
 ...
 */
export function getAlertStats(): Record<string, AlertEntry> {
