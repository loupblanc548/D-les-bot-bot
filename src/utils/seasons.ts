 ...
 */
export function formatApiSeason(chapter: string, season: string): string | null {
  const ch = parseInt(chapter, 10);
  const se = parseInt(season, 10);
  if (isNaN(ch) || isNaN(se)) return null;
  if (ch === 1) {
    return `Season ${se}`;
  }
 ...
 */
export function getAllValidSeasons(): string[] {
 ...
 */
export function seasonsMatch(a: string | null, b: string | null): boolean {
  if (!a || !b) return false;
 ...
 *   Chapter 5 S 4   → 504
 */
export function getSeasonNumericOrder(seasonTag: string): number {
 ...
 */
export function parseSeasonRange(input: string): { start: string; end: string } | null {
  const trimmed = input.trim();
  ...
  if (startOrder < 0 || endOrder < 0 || startOrder > endOrder) return null;
 ...
 */
export function normalizeSeasonRange(input: string): string | null {
...
export function isSeasonInRange(
  ...
  if (cosmeticOrder < 0 || startOrder < 0 || endOrder < 0) return false;
