...
This is a deepsec scanning workspace. Project `bot` was just registered
(target: `..`). Setup is incomplete — `data/bot/INFO.md`
...
2. **Fill in `data/bot/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id bot
pnpm deepsec process --project-id bot
```
