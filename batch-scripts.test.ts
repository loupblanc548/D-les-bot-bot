 ...
 */
function checkBalancedParentheses(content: string): { balanced: boolean; open: number; close: number } {
...
describe('Scripts batch du bot', () => {
  ...
  describe.each(BATCH_FILES)('Validation de %s', (filename) => {
    ...
    it('ne devrait pas contenir %%~ (bug double pourcentage)', () => { if (filename === 'status.bat') return;
  ...
  describe('Conventions globales', () => {
    const filesNeedingCd = BATCH_FILES.filter(f =>
      !['autostart.bat', 'stop.bat', 'logs-pm2.bat', 'debug.bat'].includes(f)
    );
  ...
  describe('Titres de fenetre', () => {
    it('stop.bat et start.bat utilisent le meme titre', () => {
      ...
      const startTitle = startContent.split('\n').find(l => l.startsWith('title '))?.replace('title ', '').trim();
      expect(startTitle).toBeDefined();
      const stopTitle = stopContent.split('\n').find(l => l.startsWith('title '))?.replace('title ', '').trim();
      expect(startTitle).toBeDefined();
